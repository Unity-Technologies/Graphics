...
@"    for(int i=0; i<Octaves; i++) {
    ...
    Out = pow(sum*0.5+0.5, 2.2);",
                    new ParameterDescriptor[]
                    {
                        ...
                        new ParameterDescriptor("randRotMat", TYPE.Mat3, Usage.Local, new float[] {0.00f,  0.80f,  0.60f, -0.80f,  0.36f, -0.48f, -0.60f, -0.48f,  0.64f}),
...
@"    for(int i=0; i<Octaves; i++) {
	...
	Out = sum;",
                ...
                new(
                    "GradientTexFBM",
@"    Position*=0.1; //adjust for texture scale - so it matches the others
    for(int i=0; i<Octaves; i++) {
