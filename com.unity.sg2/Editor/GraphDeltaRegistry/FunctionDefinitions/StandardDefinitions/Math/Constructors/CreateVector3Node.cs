using Usage = UnityEditor.ShaderGraph.GraphDelta.GraphType.Usage;

namespace UnityEditor.ShaderGraph.Defs
{

    internal class CreateVector3Node : IStandardNode
    {
        public static string Name => "CreateVector3";
        public static int Version => 1;

        public static FunctionDescriptor FunctionDescriptor => new(
            Name,
@"
    Out.x = X;
    Out.y = Y;
    Out.z = Z;
",
            new ParameterDescriptor[]
            {
                new ParameterDescriptor("X", TYPE.Float, Usage.In),
                new ParameterDescriptor("Y", TYPE.Float, Usage.In),
                new ParameterDescriptor("Z", TYPE.Float, Usage.In),
                new ParameterDescriptor("Out", TYPE.Vec3, Usage.Out)
            }
        );

        public static NodeUIDescriptor NodeUIDescriptor => new(
            Version,
            Name,
            displayName: "Create Vector 3",
            tooltip: "Creates a user-defined value with 3 channels.",
            category: "Math/Constructor",
            synonyms: new string[4] { "3", "v3", "vec3", "float3" },
            description: "pkg://Documentation~/previews/CreateVector3.md",
            parameters: new ParameterUIDescriptor[4] {
                new ParameterUIDescriptor(
                    name: "X",
                    tooltip: "the first component"
                ),
                new ParameterUIDescriptor(
                    name: "Y",
                    tooltip: "the second component"
                ),
                new ParameterUIDescriptor(
                    name: "Z",
                    tooltip: "the third component"
                ),
                new ParameterUIDescriptor(
                    name: "Out",
                    displayName: string.Empty,
                    tooltip: "a user-defined value with 3 channels"
                )
            }
        );
    }
}
