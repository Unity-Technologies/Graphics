...
namespace UnityEditor.ShaderFoundry
{
    internal class FieldPropertyContext
    {
        ...
        internal FieldPropertyContext(ShaderContainer container, ShaderType fieldType, string fieldName, IEnumerable<ShaderAttribute> attributes, PropertyAttribute propertyAttribute)
        {
            ...
            ParseDefaultValue(fieldType, attributes);
        }

        void ParseDefaultValue(ShaderType type, IEnumerable<ShaderAttribute> attributes)
        {
            var defaultAttribute = DefaultAttribute.Find(attributes);
            if (defaultAttribute == null)
                return;

            DefaultValue = defaultAttribute.BuildMaterialPropertyDefaultValue(type);
        }
    }
}
