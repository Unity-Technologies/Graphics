...
namespace UnityEditor.ShaderFoundry
{
    static class PropertyDeclarations
    {
        ...
        {
            var hasScaleOffset = !context.Attributes.FindFirst("NoScaleOffset").IsValid;
        ...
        {
            var defaultAttribute = DefaultAttribute.Find(context.Attributes);
            if (defaultAttribute != null)
            {
                var param = defaultAttribute.Parameters.GetAttributeParam(0);
                if (param.IsValid)
                {
                    context.DefaultValue = $"\"{param.Value}\" {{}}";
                }
            }
