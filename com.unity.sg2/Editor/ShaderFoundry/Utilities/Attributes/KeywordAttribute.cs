    ...
    internal class EnumKeywordAttribute
    {
        ...
        static AttributeParsing.SignatureDescription<EnumKeywordAttribute> AttributeSignature = new AttributeParsing.SignatureDescription<EnumKeywordAttribute>()
        {
            ParameterDescriptions = new List<AttributeParsing.ParameterDescription<EnumKeywordAttribute>>
            {
                ...
                new AttributeParsing.ParameterDescription<EnumKeywordAttribute>(ValuesParamName, (param, index, target) => ParseValues(param, index, target)),
            },
        ...
        static void ParseValues(ShaderAttributeParam param, int index, EnumKeywordAttribute target)
        {
            ...
            {
                ErrorHandling.ReportError($"Argument '{ValuesParamName}' must be an array.");
                return;
            }
            int enumValue = 0;
            foreach (var value in param.Values)
            {
                if (!value.ValueIsString)
                    ErrorHandling.ReportError("All sub arguments must be literals and not nested arrays");

                // TODO: Handle [Name,Value,Name,Value]
                target.AddEnum(value.Value, enumValue);
                ++enumValue;
            }
        }
