...
namespace UnityEditor.Rendering.Universal
{
    internal partial class UniversalRenderPipelineAssetUI
    {
        internal static class Styles
        {
            ...
            public static GUIContent renderTextureUVOriginStrategyText = EditorGUIUtility.TrTextContent("Render Texture UV Origin Strategy", "Controls how the render pipeline decides what approach it will use to work out if a render texture should use Unity's default origin for textures or if it should match the backbuffer.");
            public static readonly GUIContent intermediateTextureMode = EditorGUIUtility.TrTextContent("Intermediate Texture", "Controls the use of intermediate textures when rendering. If you disable them, this prevents you from using certain features.");
            public static readonly string intermediateTextureAlways = L10n.Tr("Forces rendering via an intermediate texture when any Renderer Feature is active. This option provides compatibility for Renderer Features that cannot render directly to the backbuffer or do not correctly declare their inputs. Using this setting bypasses on-tile GPU optimizations, which causes a severe performance impact on mobile and XR platforms.");
            public static readonly string intermediateTextureNeverWithoutIssue = L10n.Tr("Disables features that require separate passes to avoid costly memory operations. Note: On Tiled GPUs (mobile/XR), this keeps rendering on-tile for a major performance boost.");
            public static readonly string intermediateTextureNeverWithIssue = L10n.Tr("Disables features that require separate passes to avoid costly memory operations. Note: On Tiled GPUs (mobile/XR), this keeps rendering on-tile for a major performance boost.\nSome active features are being ignored to maintain performance.");
            public static readonly string intermediateTextureAutoWithoutIssue = L10n.Tr("Optimal path enabled. No features currently use intermediate textures.");
            public static readonly string intermediateTextureAutoWithIssue = L10n.Tr("Performance path blocked. One or more active features require intermediate textures.");
            public static readonly string logFeature = L10n.Tr("Log Features");
            public static readonly string logBlockedFeature = L10n.Tr("Log Blocked Features");
            public static readonly string intermediateTextureModeNeverTooltip = L10n.Tr("Disabled due to Intermediate Texture being disabled.");

            public static GUIContent GetNoIntermediateTextureVariant(GUIContent former)
                => EditorGUIUtility.TrTextContent(former.text, $"{former.tooltip} {intermediateTextureModeNeverTooltip}");
