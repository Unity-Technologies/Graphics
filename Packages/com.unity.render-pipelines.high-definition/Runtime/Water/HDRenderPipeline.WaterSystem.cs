...
namespace UnityEngine.Rendering.HighDefinition
{
    public partial class HDRenderPipeline
    {
        ...
        float EvaluateCausticsMaxLOD(WaterSurface.WaterCausticsResolution resolution)
        {
            switch (resolution)
            {
            ...
            WaterRenderingParameters parameters)
        {
            ...
            bool supportIndirectGPU = SystemInfo.graphicsDeviceType != GraphicsDeviceType.Metal;

            DrawWaterSurface(cmd, parameters, k_WaterGBufferPass, supportIndirectGPU, patchDataBuffer, indirectBuffer, cameraFrustumBuffer);
            ...
            in WaterGBuffer waterGBuffer, in BuildGPULightListOutput lightLists)
        {
            ...
            if (!ShouldRenderWater(hdCamera) || !waterGBuffer.valid)
                return;
            ...
            using (var builder = renderGraph.AddRenderPass<WaterRenderingDeferredData>("Render Water Surfaces Deferred", out var passData, ProfilingSampler.Get(HDProfileId.WaterSurfaceRenderingDeferred)))
            {
                ...
                // GBuffer data
                passData.indirectBuffer = builder.ReadBuffer(waterGBuffer.indirectBuffer);
                passData.tileBuffer = builder.ReadBuffer(waterGBuffer.tileBuffer);
        ...
        void WaterRejectionTag(RenderGraph renderGraph, CullingResults cull, HDCamera hdCamera, TextureHandle depthBuffer)
        {
            ...
            using (var builder = renderGraph.AddRenderPass<WaterExclusionPassData>("Water Exclusion", out var passData, ProfilingSampler.Get(HDProfileId.WaterExclusion)))
            {
                ...
                builder.SetRenderFunc(
                    ...
                    {
                        ctx.cmd.SetGlobalInteger("_StencilWriteMaskStencilTag", (int)StencilUsage.WaterExclusion);
                        ctx.cmd.SetGlobalInteger("_StencilRefMaskStencilTag", (int)StencilUsage.WaterExclusion);
