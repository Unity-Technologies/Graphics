...
| Background Clouds | When enabled, the occlusion is attenuated by the Background Clouds used on the Visual Environnement (Cloud layer). |
| Volumetric Clouds | When enabled, HDRP uses the volumetric clouds texture to occlude the lens flare. HDRP always considers the lens flare to be behind the volumetric clouds because it calculates occlusion in screen space. |
| Water | When enabled, HDRP uses the Water Rendering (in screen space) for the occlusion. |
