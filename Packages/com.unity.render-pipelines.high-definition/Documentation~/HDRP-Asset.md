...
| **- STP Quality Mode**                      | Configures STP to prioritize quality over performance. |
| **- STP Responsive Feature**                | Configures STP to avoid causing artifacts on transparent objects such as sparks or dust particles. |
...
| **- Dynamic Resolution Step Size**          | The size of each step (as a percentage) between unique resolutions supported by dynamic resolution. This value ultimately determines the total number of unique resolutions that the dynamic resolution system will use. Smaller steps means smoother transitions between resolutions, but it also means increased overhead and increased memory usage for some upscaling filters. |
