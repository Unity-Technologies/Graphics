...
### Fixed
- Removed unnecessary file access when new assets are imported.
...
- Fixed an exception in the console when dragging a texture asset from the project browser to a Texture field in an Output context.
- Fixed the UI of dropdown settings so they are no longer cropped.
- Fixed the VFX component inspector because it had an unpaired `EditorGUI.BeginChangeCheck` that was breaking the state of `GUI.changed`.
- Fixed a serialization issue causing a potential crash due to bounds computation.
- Fixed a VFX instance leak due to the asset editor not being properly destroyed.
...
- Fixed compilation error when targeting .NET Standard runtime.
- Added extra memory to allow external threads to steal VFX update jobs.
- Fixed VFX instance leak due to asset editor not properly destroyed.
- Fixed an exception in the console when dragging a texture asset from the project browser to a Texture field in an Output context
- Dropdown could be cropped when the text was too long
...
- Fixed errors in the console when undoing changes from gizmo in some specific conditions
- Fix serialization issue causing a potential crash due to bounds computation
- Fixed issue where VFXs were not culled properly, causing flickering shadows.
- Fix mesh LOD flickering when using TAA
- Fix mismatching LOD between eyes in multi-pass VR
- Restore some tooltips that were lost
- Fix flickering shadows in URP
