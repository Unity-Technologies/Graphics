...
public class InstancedRenderingTest : MonoBehaviour
{
    ...
    private void DoRender(CommandBuffer cmd, bool dontDraw = false)
    {
        ...
        // HDRP uses Equal for depth test when drawing in forward pass where the CommandBuffer-version of the apis are using. However the implementation of this test
        // for CommandBuffer drawing doesn't do a depth rendering. Use LessEqual for depth testing.
        if (testApi is TestAPI.CommandBufferDrawMeshInstanced or TestAPI.CommandBufferDrawMeshInstancedIndirect or TestAPI.CommandBufferDrawMeshInstancedProcedural
            or TestAPI.CommandBufferDrawProcedural or TestAPI.CommandBufferDrawProceduralIndirect)
            m_ClonedMaterial.SetInt("_ZTestDepthEqualForOpaque", (int)CompareFunction.LessEqual);
        ...
        int forwardPassIndex = material.FindPass("Forward");
#if UNITY_EDITOR
        if (UseImmediateRendering)
        {
