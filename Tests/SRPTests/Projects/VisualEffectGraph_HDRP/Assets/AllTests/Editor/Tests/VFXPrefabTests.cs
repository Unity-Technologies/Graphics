    ...
    public class VisualEffectPrefabTest
    {
        ...
        public IEnumerator Create_Prefab_And_Check_UndoRedo()
        {
            ...
            //Testing Undo/Redo on PropertySheet (see https://fogbugz.unity3d.com/f/cases/1395584/)
            {
                ...
                Assert.AreEqual(property.intValue, 666);

                property.intValue = 432;

                Undo.IncrementCurrentGroup();
                editor.serializedObject.ApplyModifiedProperties();

                editor.serializedObject.Update();
                Assert.AreEqual(property.intValue, 432);

                Undo.PerformUndo();
                editor.serializedObject.Update();
                Assert.AreEqual(property.intValue, 666, "Regression from case 1395584");
            }
