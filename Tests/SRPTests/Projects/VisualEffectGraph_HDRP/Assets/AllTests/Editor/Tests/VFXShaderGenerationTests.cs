...
namespace UnityEditor.VFX.Test
{
    ...
    public class VFXShaderGenerationTests
    {
        ...
        string ShaderGraphOutputPrepare(string shaderGraphPath, VFXAbstractRenderedOutput.BlendMode blendMode, VFXAbstractParticleOutput.SortMode sortMode = VFXAbstractParticleOutput.SortMode.Auto)
        {
            var graph = VFXTestCommon.MakeTemporaryGraph();
            var spawnerContext = ScriptableObject.CreateInstance<VFXBasicSpawner>();
            var initContext = ScriptableObject.CreateInstance<VFXBasicInitialize>();
        ...
        {
            var vfxPath = ShaderGraphOutputPrepare("Assets/AllTests/VFXTests/GraphicsTests/Shadergraph/Unlit/sg-for-autotest-unlit-opaque.shadergraph_1", VFXAbstractRenderedOutput.BlendMode.Alpha, VFXAbstractParticleOutput.SortMode.On);
        ...
        {
            var vfxPath = ShaderGraphOutputPrepare(null, VFXAbstractRenderedOutput.BlendMode.Opaque, VFXAbstractParticleOutput.SortMode.On);
            ShaderGraphSortingVerify(vfxPath, true);
        }
        ...
        {
            var vfxPath = ShaderGraphOutputPrepare("Assets/AllTests/VFXTests/GraphicsTests/Shadergraph/Unlit/sg-for-autotest-unlit-alphablend.shadergraph_1", VFXAbstractRenderedOutput.BlendMode.Alpha, VFXAbstractParticleOutput.SortMode.On);
            var vfxAsset = AssetDatabase.LoadAssetAtPath<VisualEffectAsset>(vfxPath);
            var outputContext = ((VFXGraph)vfxAsset.GetResource().graph).children.OfType<VFXPlanarPrimitiveOutput>().FirstOrDefault();
