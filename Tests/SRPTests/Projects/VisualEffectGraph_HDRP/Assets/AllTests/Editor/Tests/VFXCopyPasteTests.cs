...
namespace UnityEditor.VFX.Test
{
    ...
    public class VFXCopyPasteTests
    {
        ...
        public IEnumerator PasteMissingPointCacheAsset()
        {
            ...
            // Create one system
            var spawner = VFXTestCommon.CreateSpawners(view, m_ViewController, 1).Single();
            ...
            var output = m_ViewController.AddVFXContext(Vector2.zero, contextOutputDesc);
            var init = m_ViewController.AddVFXContext(Vector2.zero, contextInitializeDesc);
            var flowEdge = new VFXFlowEdgeController(
                    ((VFXContextController)m_ViewController.GetNewNodeController(output)).flowInputAnchors.FirstOrDefault(),
                    ((VFXContextController)m_ViewController.GetNewNodeController(init)).flowOutputAnchors.FirstOrDefault());
            ...
            var setPositionBlock = VFXLibrary.GetBlocks().First(x => x.name.StartsWith("Set Position from Map")).CreateInstance() as AttributeFromMap;
