...
namespace PerformanceTests.Runtime
{
    public class RenderGraphPerformanceTests
    {
        ...
        readonly OnTileValidationLayer m_OnTileValidationLayer = new();
        Camera m_Camera;
        ...
        public enum TestCase
        {
            ...
            ComplexPassOnTileValidation,
        }

        public enum TestCaseOnTile
        {
            NoValidation,
            WithValidation
        }
        ...
        // This is a [Test] because we don't want to measure overhead from executing a full frame.
        [Test, Performance]
        public void RecordAndExecute_OnTileRenderer_MeasureTotal([Values] TestCaseOnTile testCase)
        {
            // Prevent accidental usage
            m_NumPasses = 0;

            Measure.Method(() => TestBodyOnTile(testCase))
                .WarmupCount(k_NumWarmupIterations)
                .MeasurementCount(k_NumMeasurements)
                .Run();
        }
        ...
        void TestBody(TestCase testCase, RenderTextureUVOriginStrategy strategy)
        {
            ...
            switch (testCase)
            {
                ...
                case TestCase.SimplePassWithCulledPasses:
                    ...
                    break;
                case TestCase.ComplexPassOnTileValidation:
                    RecordComplexPassesOnTileValidationLayer();
                    break;
                default:
        ...
        void TestBodyOnTile(TestCaseOnTile testCase)
        {
            RenderGraphParameters rgParams = new()
            {
                executionId = m_Camera.GetEntityId(),
            ...
            switch (testCase)
            {
                case TestCaseOnTile.NoValidation:
                    RecordOnTileRendererPasses();
                    break;
                case TestCaseOnTile.WithValidation:
                    RecordOnTileRendererPassesOnTileValidationLayer();
                    break;
                default:
                    throw new NotImplementedException();
            }
        ...
        TestRenderTargets ImportAndCreateRenderTargetsComplexPass(RenderGraph g)
        {
        ...
        void AddComplexPasses(TestRenderTargets targets)
        {
        ...
        void RecordComplexPasses()
        {
            using (new ProfilingScope(k_RecordRenderGraphSampler))
            {
                TestRenderTargets targets = ImportAndCreateRenderTargetsComplexPass(m_RenderGraph);

                AddComplexPasses(targets);
            }
        }

        void RecordComplexPassesOnTileValidationLayer()
        {
            using (new ProfilingScope(k_RecordRenderGraphSampler))
            {
                m_RenderGraph.validationLayer = m_OnTileValidationLayer;
                m_OnTileValidationLayer.renderGraph = m_RenderGraph;

                TestRenderTargets targets = ImportAndCreateRenderTargetsComplexPass(m_RenderGraph);
                
                m_OnTileValidationLayer.Add(targets.colorMaps[0]);
                m_OnTileValidationLayer.Add(targets.colorMaps1[0]);
                m_OnTileValidationLayer.Add(targets.colorMaps2[0]);
                
                AddComplexPasses(targets);
            }
        }

        void AddOnTileRendererPasses(TestRenderTargets targets)
        {
            void AddComplexPass(TestRenderTargets targets, int index, int input, int output)
            {
                using (var builder = m_RenderGraph.AddRasterRenderPass<ComplexPassData>($"Complex Pass {index}", out var passData))
                {
                    builder.SetRenderAttachment(targets.backBuffer, 0, AccessFlags.Write);
                    builder.SetRenderAttachment(targets.colorMaps[output], 1, AccessFlags.Write);
                    builder.SetRenderAttachment(targets.colorMaps1[output], 2, AccessFlags.Write);
                    builder.SetRenderAttachment(targets.colorMaps2[output], 3, AccessFlags.Write);                    
                    builder.SetRenderAttachmentDepth(targets.depthBuffer, AccessFlags.Write);
            ...
            using (var builder = m_RenderGraph.AddRasterRenderPass<ComplexPassData>("Setup Pass 1", out var passData))
            {
                builder.AllowPassCulling(false);
                builder.SetRenderFunc(static (ComplexPassData data, RasterGraphContext context) => { });
            }  
            
            using (var builder = m_RenderGraph.AddRasterRenderPass<ComplexPassData>("Setup Pass 2", out var passData))
            {
                builder.AllowPassCulling(false);
                builder.SetRenderFunc(static (ComplexPassData data, RasterGraphContext context) => { });
            }

            using (var builder = m_RenderGraph.AddRasterRenderPass<ComplexPassData>("Setup Pass 3", out var passData))
            {
                builder.AllowPassCulling(false);
                builder.SetRenderFunc(static (ComplexPassData data, RasterGraphContext context) => { });
            }
            ...
            var lut = m_RenderGraph.CreateTexture(new TextureDesc(1024, 128)
            {
                colorFormat = GraphicsFormat.R8G8B8A8_UInt,
                name = "LUT"
            });

            using (var builder = m_RenderGraph.AddUnsafePass<ComplexPassData>("LUT Pass", out var passData))
            {
                builder.SetRenderAttachment(lut, 0, AccessFlags.WriteAll);
                builder.AllowPassCulling(false);
                builder.SetRenderFunc(static (ComplexPassData data, UnsafeGraphContext context) => { });
            }

            using (var builder = m_RenderGraph.AddComputePass<ComplexPassData>("Compute Pass", out var passData))
            {
                builder.AllowPassCulling(false);
                builder.SetRenderFunc(static (ComplexPassData data, ComputeGraphContext context) => { });
            }

            // Custom Gbuffer prepass
            AddComplexPass(targets, 0, -1, 0);
            // Gbuffer pass
            AddComplexPass(targets, 1, -1, 0);
            // Deferred Lighting pass (use input attachment)
            AddComplexPass(targets, 2, 0, 1);
            // Custom additional lighting pass
            AddComplexPass(targets, 3, -1, 1);

            using (var builder = m_RenderGraph.AddRasterRenderPass<ComplexPassData>($"Forward Opaque", out var passData))
            {
                builder.SetRenderAttachment(targets.backBuffer, 0, AccessFlags.Write);
                builder.SetRenderAttachmentDepth(targets.depthBuffer, AccessFlags.Write);
            ...
            using (var builder = m_RenderGraph.AddRasterRenderPass<ComplexPassData>($"Transparent Opaque", out var passData))
            {
                builder.SetRenderAttachment(targets.backBuffer, 0, AccessFlags.Write);
                builder.SetRenderAttachmentDepth(targets.depthBuffer, AccessFlags.Write);
            ...
            using (var builder = m_RenderGraph.AddRasterRenderPass<ComplexPassData>($"Post Processing", out var passData))
            {
                builder.SetInputAttachment(targets.backBuffer, 0);
                builder.SetRenderAttachment(targets.colorMaps[0],0, AccessFlags.WriteAll);
            ...
            using (var builder = m_RenderGraph.AddRasterRenderPass<ComplexPassData>($"Post Processing 2", out var passData))
            {
                builder.SetInputAttachment(targets.colorMaps[0], 0);
                builder.SetRenderAttachment(targets.backBuffer, 0, AccessFlags.WriteAll);

                builder.UseTexture(lut, AccessFlags.Read);
        ...
        void RecordOnTileRendererPasses()
        {
            using (new ProfilingScope(k_RecordRenderGraphSampler))
            {
                TestRenderTargets targets = ImportAndCreateRenderTargetsComplexPass(m_RenderGraph);
                                
                AddOnTileRendererPasses(targets);
            }
        }

        void RecordOnTileRendererPassesOnTileValidationLayer()
        {
            using (new ProfilingScope(k_RecordRenderGraphSampler))
            {
                m_RenderGraph.validationLayer = m_OnTileValidationLayer;
