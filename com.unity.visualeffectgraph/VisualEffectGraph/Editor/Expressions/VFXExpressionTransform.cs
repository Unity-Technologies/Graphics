...
namespace UnityEditor.VFX
{
    ...
    class VFXExpressionVector3sToMatrix : VFXExpression
    {
        ...
        public override string GetCodeString(string[] parents)
        {
            return string.Format("float4x4(float4({0}, 0.0), float4({0}, 0.0), float4({0}, 0.0), float4({0}, 1.0));", parents[0], parents[1], parents[2], parents[3]);
        }
    }
    ...
    class VFXExpressionVector4sToMatrix : VFXExpression
    {
        ...
        public override string GetCodeString(string[] parents)
        {
            return string.Format("float4x4({0}, {0}, {0}, {0});", parents[0], parents[1], parents[2], parents[3]);
        }
    }
