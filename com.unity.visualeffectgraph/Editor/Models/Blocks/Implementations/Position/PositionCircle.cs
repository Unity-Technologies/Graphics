        ...
        public override IEnumerable<VFXNamedExpression> parameters
        {
            get
            {
                ...
                if (spawnMode == SpawnMode.Random)
                    theta = arcCircle_arc * new VFXExpressionRandom(true, this, 0);
                else
                    theta = arcCircle_arc * arcSequencer;

                var one = VFXOperatorUtility.OneExpression[UnityEngine.VFX.VFXValueType.Float];

                var rNorm = VFXOperatorUtility.Sqrt(volumeFactor + (one - volumeFactor) * new VFXExpressionRandom(true, this, 1)) * arcCircleRadius;
