...
class VisualEffectAssetEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (!serializedObject.isEditingMultipleObjects)
        {
            ...
            string directory = Path.GetDirectoryName(assetPath) + "/" + VFXAssetProcessor.k_ShaderDirectory + "/" + asset.name + "/";
            ...
            foreach (var shader in objects)
            {
                if (shader is Shader || shader is ComputeShader)
                {
                    ...
                    int buttonsWidth = VFXAssetProcessor.allowExternalization ? 240 : 160;
                    ...
                    if (index >= 0)
                    {
                        if (VFXAssetProcessor.allowExternalization && index < resource.GetShaderSourceCount())
                        {
                            string shaderSourceName = resource.GetShaderSourceName(index);
                            string externalPath = directory + shaderSourceName;

                            externalPath = directory + shaderSourceName.Replace('/', '_') + VFXAssetProcessor.k_ShaderExt;
