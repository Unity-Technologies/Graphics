    ...
    class SubGraphOutputNode : AbstractMaterialNode, IHasSettings
    {
        ...
        void ValidateSlotType()
        {
            ...
            else if (!s_ValidSlotTypes.Contains(slots.FirstOrDefault().concreteValueType))
            {
                ...
                owner.AddValidationError(tempId, "Preview can only compile if the first output slot is a Vector, Matrix, or Boolean type. Please adjust slot types.", ShaderCompilerMessageSeverity.Error);
            }
        }
