...
namespace UnityEditor.Graphing
{
    ...
    class ShaderGUIOverridePropertyDrawer : IPropertyDrawer
    {
        ...
        {
            if (shaderGuiOverrideInfo.OverrideEnabled && shaderGuiOverrideInfo.ShaderGUIOverride != null && !ValidCustomEditorType(shaderGuiOverrideInfo.ShaderGUIOverride))
            {
                m_MasterNode.owner.messageManager?.ClearNodesFromProvider(m_MasterNode, m_MasterNode.ToEnumerable());
                m_MasterNode.owner.messageManager?.AddOrAppendError(m_MasterNode, m_MasterNode.guid,
                    new ShaderMessage(string.Format(k_InvalidShaderGUI, shaderGuiOverrideInfo.ShaderGUIOverride), ShaderCompilerMessageSeverity.Warning));
            }
            else
            {
                m_MasterNode.owner.messageManager?.ClearNodesFromProvider(m_MasterNode, m_MasterNode.ToEnumerable());
