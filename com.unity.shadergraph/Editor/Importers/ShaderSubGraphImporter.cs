    ...
    class ShaderSubGraphImporter : ScriptedImporter
    {
        ...
        static void ProcessSubGraph(SubGraphAsset asset, GraphData graph)
        {
            ...
            foreach (var node in nodes)
            {
                ...
                // This is a stop-gap to prevent the autogenerated guids from JsonObject and ShaderInput from
                ...
                for (int i = previousPropertyCount; i < collector.properties.Count; ++i)
                {
                    ...
                    // Generate a deterministic guid to avoid non-determeinistic reimports, using RFC 4122 §4.3 version 5.
                    var namespaceId = Encoding.UTF8.GetBytes(node.objectId);
                    FlipByNetworkOrder(namespaceId);
                    var name = Encoding.UTF8.GetBytes(newName);
                    var hash = SHA1.Create().ComputeHash(namespaceId.Concat(name).ToArray());
                    byte[] newguid = new byte[16];
                    Array.Copy(hash, newguid, 16); // truncate hash
                    newguid[6] = (byte)((newguid[6] & 0x0F) | 0x80);
                    newguid[8] = (byte)((newguid[8] & 0x3F) | 0x80);
                    FlipByNetworkOrder(newguid);

                    // override the autogenerated guids that are resulting in non-deterministic reimports.
                    prop.OverrideObjectId(newName);
