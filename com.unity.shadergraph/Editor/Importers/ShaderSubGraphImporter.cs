    ...
    class ShaderSubGraphImporter : ScriptedImporter
    {
        ...
        static void ProcessSubGraph(SubGraphAsset asset, GraphData graph)
        {
            ...
            foreach (var node in nodes)
            {
                ...
                int newPropertyCount = collector.properties.Count;
                ...
                for (int i = previousPropertyCount; i < collector.properties.Count; ++i)
                {
                    ...
                    byte[] GenerateNamespaceUUID(string Namespace, string Name)
                    {
                    ...
                    // If the node's object Id is not actually a guid, use the assetGuid as a namespace instead.
                    if (!Guid.TryParse(namespaceId, out _))
                    {
                        Debug.LogWarning("Node objectId is not a valid Guid, using assetGuid instead." + node.objectId);
                        nameId += "_" + node.objectId; // Pad the name with the object Id.
                        namespaceId = asset.assetGuid;
                        
                    }
                    
                    // Mix-in the property Index and differentiate between guid/objectId.
                    var newObjectId = GenerateNamespaceUUID(node.objectId, nameId + "_ObjectId_" + i);
                    var newGuid = GenerateNamespaceUUID(node.objectId, nameId + "_Guid_" + i);

                    // override the autogenerated fields that are resulting in non-deterministic reimports.
                    prop.OverrideObjectId(new Guid(newObjectId).ToString("N"));
                    prop.OverrideGuid(new Guid(newGuid));
                }
            }
