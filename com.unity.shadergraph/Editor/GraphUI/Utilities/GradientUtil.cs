        ...
        public static string GetGradientValue(Gradient gradient, string delimiter = ";")
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                if (i < gradient.colorKeys.Length)
                    colorKeys += $"$precision4({NodeUtil.FloatToShaderValue(gradient.colorKeys[i].color.r)}, " +
                        $"{NodeUtil.FloatToShaderValue(gradient.colorKeys[i].color.g)}, " +
                        $"{NodeUtil.FloatToShaderValue(gradient.colorKeys[i].color.b)}, " +
                        $"{NodeUtil.FloatToShaderValue(gradient.colorKeys[i].time)})";
                else
            ...
            for (int i = 0; i < 8; i++)
            {
                if (i < gradient.alphaKeys.Length)
                    alphaKeys += $"$precision2({NodeUtil.FloatToShaderValue(gradient.alphaKeys[i].alpha)}, {NodeUtil.FloatToShaderValue(gradient.alphaKeys[i].time)})";
                else
