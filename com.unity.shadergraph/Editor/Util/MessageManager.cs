...
namespace UnityEditor.Graphing.Util
{
    ...
    {
        protected Dictionary<object, Dictionary<AbstractMaterialNode, List<ShaderMessage>>> m_Messages =
            new Dictionary<object, Dictionary<AbstractMaterialNode, List<ShaderMessage>>>();

        Dictionary<AbstractMaterialNode, List<ShaderMessage>> m_Combined = new Dictionary<AbstractMaterialNode, List<ShaderMessage>>();

        public bool nodeMessagesChanged { get; private set; }

        Dictionary<AbstractMaterialNode, List<ShaderMessage>> m_FoundMessages;

        public void AddOrAppendError(object errorProvider, AbstractMaterialNode nodeId, ShaderMessage error)
        {
            if (!m_Messages.TryGetValue(errorProvider, out var messages))
            {
                messages = new Dictionary<AbstractMaterialNode, List<ShaderMessage>>();
        ...
        public IEnumerable<KeyValuePair<AbstractMaterialNode, List<ShaderMessage>>> GetNodeMessages()
        {
            var fixedNodes = new List<AbstractMaterialNode>();
            m_Combined.Clear();
        ...
        public void RemoveNode(AbstractMaterialNode nodeId)
        {
            foreach (var messageMap in m_Messages)
            {
        ...
        public void ClearNodesFromProvider(object messageProvider, IEnumerable<AbstractMaterialNode> nodes)
        {
            if (m_Messages.TryGetValue(messageProvider, out m_FoundMessages))
            {
                ...
                {
                    if (m_FoundMessages.TryGetValue(node, out var messages))
                    {
        ...
        void DebugPrint()
        {
            ...
            foreach (var messageMap in m_Messages)
            {
                ...
                foreach (var messageList in messageMap.Value)
                {
                    output.AppendFormat("\t\tNode {0} has {1} messages:\n", messageList.Key.name, messageList.Value.Count);
