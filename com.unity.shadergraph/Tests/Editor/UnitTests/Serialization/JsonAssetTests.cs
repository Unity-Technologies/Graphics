...
namespace UnitTests.Serialization
{
    ...
    class JsonAssetTests
    {
        ...
        public void Single()
        {
            ...
            var str = $"--- {expected[0].type} {expected[0].id}{Environment.NewLine}{expected[0].json}";
        ...
        public void Multiple()
        {
            var expected = new []
            {
                ...
                new RawJsonObject
                {
                    type = typeof(AddNode).FullName,
                    id = "89b18fe3-43b5-4354-9cb1-d5e76c39988e",
                    json = "{a value goes here}"
                },
                new RawJsonObject
                {
                    type = typeof(Vector4MaterialSlot).FullName,
            ...
            {
                str += $"--- {rawJsonObject.type} {rawJsonObject.id}{Environment.NewLine}{rawJsonObject.json}{Environment.NewLine}{Environment.NewLine}";
            }
            var result = JsonAsset.Parse(str);
            Assert.AreEqual(expected, result);
        }
