...
void ImposterSample(in bool parallaxCheck, in ImposterData imp, in SamplerState ss, in float4 texelSize, in Texture2D albedoMap, in Texture2D normalMap, in float LOD, out float4 outAlbedo)//, out float depth )
{
    ...
    {
        //add parallax shift 
        float depth0 = SAMPLE_TEXTURE2D_LOD(normalMap, ss, vp0uv, LOD).a;
        float depth1 = SAMPLE_TEXTURE2D_LOD(normalMap, ss, vp1uv, LOD).a;
        float depth2 = SAMPLE_TEXTURE2D_LOD(normalMap, ss, vp2uv, LOD).a;
    ...
    float2 border = texelSize.xy * _ImposterBorderClamp;
...
void ImposterSample_CurrentFrame(in bool parallaxCheck, in ImposterData imp, in SamplerState ss, in float4 texelSize, in Texture2D albedoMap, in Texture2D normalMap, in float LOD, out float4 outAlbedo)//, out float depth )
{
    ...
    outAlbedo = SAMPLE_TEXTURE2D_GRAD(albedoMap, ss, vp0uv, ddxy.xy, ddxy.zw);
    //outNormal = SAMPLE_TEXTURE2D_GRAD(normalMap, ss, vp0uv, ddxy.xy, ddxy.zw);
}

void Imposter(in bool parallaxCheck, in float3 inPositionOS, in float2 inUV, in SamplerState ss, in float4 ts, in Texture2D albedoMap, in Texture2D normalMap, in float LOD,
    out float3 outPositionOS, out float4 outColor/*, out float3 outNormal, out float outAlpha*/)
{
    ImposterData imp;
    ...
    //ImposterSample(imp, ss, ts, albedoMap, normalMap, LOD, albedoMapSample, normalMapSample);
    ImposterSample(parallaxCheck, imp, ss, ts, albedoMap, normalMap, LOD, albedoMapSample);

    outColor = albedoMapSample;
    //outNormal = normalMapSample.xyz * 2 - 1;
    //outAlpha = albedoMapSample.a;
}

void Imposter_Current(in bool parallaxCheck, in float3 inPositionOS, in float2 inUV, in SamplerState ss, in float4 ts, in Texture2D albedoMap, in Texture2D normalMap, in float LOD,
    out float3 outPositionOS, out float4 outColor/*, out float3 outNormal, out float outAlpha*/)
{
    ImposterData imp;
    imp.vertex = inPositionOS;
    ...
    half4 albedoMapSample;
    //ImposterSample(imp, ss, ts, albedoMap, normalMap, LOD, albedoMapSample, normalMapSample);
    ImposterSample_CurrentFrame(parallaxCheck, imp, ss, ts, albedoMap, normalMap, LOD, albedoMapSample);
