...
float3 SpeedTreeWind(float3 vPos, float3 vNormal, float4 vTexcoord0, float4 vTexcoord1, float4 vTexcoord2, float4 vTexcoord3, int iWindQuality, bool bBillboard)
{
	...
	if (iWindQuality > 0)
	{
        float3 rotatedWindVector = TransformObjectToWorldDir(_ST_WindVector.xyz);
		...
		float3 treePos = float3(matObjectToWorld[0].w, matObjectToWorld[1].w, matObjectToWorld[2].w);
		...
		if (!bBillboard)
		{
			...
			if (geometryType > ST_GEOM_TYPE_FROND)
			{
				...
				if (geometryType == ST_GEOM_TYPE_FACINGLEAF)
				{
					...
					float offsetLen = length(vReturnPos);
					vReturnPos = mul(vReturnPos, (float3x3)UNITY_MATRIX_I_M); // inv(MV) * vReturnPos
					vReturnPos = normalize(vReturnPos) * offsetLen; // make sure the offset vector is still scaled
				}
			...
			{
				float3 rotatedBranchAnchor = TransformObjectToWorldNormal(_ST_WindBranchAnchor.xyz) * _ST_WindBranchAnchor.w;
