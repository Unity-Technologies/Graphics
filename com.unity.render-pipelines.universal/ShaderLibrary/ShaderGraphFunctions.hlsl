...
float3 shadergraph_LWReflectionProbe(float3 viewDir, float3 normalOS, float lod)
{
    ...
    return DecodeHDREnvironment(SAMPLE_TEXTURECUBE_LOD(unity_SpecCube0, samplerunity_SpecCube0, reflectVec, lod), unity_SpecCube0_HDR) * GetCurrentExposureMultiplier();
}
