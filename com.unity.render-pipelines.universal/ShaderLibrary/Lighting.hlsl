...
{
    float weightProbe0 = unity_SpecCube0_BoxMin.w;
    ...
    irradiance = encodedIrradiance.rgb;//;+= weightProbe0 * encodedIrradiance.rbg;
#else
    irradiance = DecodeHDREnvironment(encodedIrradiance, unity_SpecCube0_HDR);
#endif // UNITY_USE_NATIVE_HDR || UNITY_DOTS_INSTANCING_ENABLED

    [branch] if (weightProbe0 < .9999)
    {
...
#if defined(UNITY_USE_NATIVE_HDR) || defined(UNITY_DOTS_INSTANCING_ENABLED)
        half3 env1 = encodedIrradiance.rgb;// += (1 - weightProbe1) * encodedIrradiance.rbg;
#else
        half3 env1 = DecodeHDREnvironment(encodedIrradiance, unity_SpecCube1_HDR);
#endif // UNITY_USE_NATIVE_HDR || UNITY_DOTS_INSTANCING_ENABLED

        irradiance = lerp(env1, irradiance, weightProbe0);
    }
    else
    {
    }
