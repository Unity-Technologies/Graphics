...
half4 BlendTexture(TEXTURE2D(_Texture), SAMPLER(sampler_Texture), float2 uv, float3 blendUv)
{
...
half3 SampleNormalTS(float2 uv, float3 blendUv, TEXTURE2D(bumpMap), SAMPLER(sampler_bumpMap), half scale = 1.0h)
{
#if defined(_NORMALMAP)
    half4 n = BlendTexture(bumpMap, sampler_bumpMap, uv, blendUv);
