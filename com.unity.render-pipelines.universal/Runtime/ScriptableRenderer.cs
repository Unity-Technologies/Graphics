...
namespace UnityEngine.Rendering.Universal
{
    ...
    public abstract partial class ScriptableRenderer : IDisposable
    {
            ...
            ref RenderingData renderingData)
        {
            ...
            {
                if (IsRenderPassEnabled(renderPass) && cameraData.cameraType == CameraType.Game)
                    NativeRenderPass_Configure(cmd, renderPass, cameraData);
                else
            ...
            if (IsRenderPassEnabled(renderPass) && cameraData.cameraType == CameraType.Game)
                NativeRenderPass_Execute(context, renderPass, cameraData, ref renderingData);
            else
                renderPass.Execute(context, ref renderingData);
        }
        ...
        void SetRenderPassAttachments(CommandBuffer cmd, ScriptableRenderPass renderPass, ref CameraData cameraData)
        {
            ...
            if (RenderingUtils.IsMRT(renderPass.colorAttachments))
            {
                ...
                NativeRenderPass_SetMRTAttachmentsList(renderPass, ref cameraData, validColorBuffersCount, needCustomCameraColorClear, needCustomCameraDepthClear);
            ...
            else
            {
                ...
                if (IsRenderPassEnabled(renderPass) && cameraData.cameraType == CameraType.Game)
                {
                    NativeRenderPass_SetAttachmentList(renderPass, ref cameraData, passColorAttachment, passDepthAttachment, finalClearFlag, finalClearColor);
                }
                else
                {
                    // Only setup render target if current render pass attachments are different from the active ones
