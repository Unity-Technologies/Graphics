...
namespace UnityEngine.Rendering.Universal
{
    ...
    public abstract partial class ScriptableRenderer : IDisposable
    {
        ...
        public ScriptableRenderer(ScriptableRendererData data)
        {
            ...
            NativeRenderPass.m_MergeableRenderPassesMapArrays = new int[NativeRenderPass.kRenderPassMapSize][];
            for (int i = 0; i < NativeRenderPass.kRenderPassMapSize; ++i)
            {
                NativeRenderPass.m_MergeableRenderPassesMapArrays[i] = new int[NativeRenderPass.kRenderPassMaxCount];
                for (int j = 0; j < NativeRenderPass.kRenderPassMaxCount; ++j)
                {
                    NativeRenderPass.m_MergeableRenderPassesMapArrays[i][j] = -1;
                }
            }
        ...
        private void SetPassesSceneInfo()
        {
            // Go through all the passes, set scene index and mark the final one as last pass

            int lastPassIndex = m_ActiveRenderPassQueue.Count - 1;

            // Make sure the list is already sorted!

            // reset all the passes last pass flag and set the current scene index
        ...
        public void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            using (new ProfilingScope(cmdScope, profilingExecute))
            {
                ...
                if (useRenderPassEnabled)
                    NativeRenderPass.SetupFrameData(cameraData, m_ActiveRenderPassQueue);
            ...
            ref RenderingData renderingData)
        {
            ...
            if (IsRenderPassEnabled(renderPass) && cameraData.cameraType == CameraType.Game)
            {
                NativeRenderPass.Execute(context,  renderPass, cameraData, ref  renderingData, m_ActiveRenderPassQueue,
        ...
        void SetRenderPassAttachments(CommandBuffer cmd, ScriptableRenderPass renderPass, ref CameraData cameraData)
        {
            ...
            if (RenderingUtils.IsMRT(renderPass.colorAttachments))
            {
                ...
                if (IsRenderPassEnabled(renderPass) && cameraData.cameraType == CameraType.Game)
                {
                    NativeRenderPass.SetMRTAttachmentsList(renderPass, ref  cameraData, validColorBuffersCount,  needCustomCameraColorClear,
                        needCustomCameraDepthClear, m_ActiveRenderPassQueue, ref m_ActiveColorAttachmentDescriptors,
            ...
            else
            {
                ...
                if (IsRenderPassEnabled(renderPass) && cameraData.cameraType == CameraType.Game)
                {
                    NativeRenderPass.SetAttachmentList(renderPass, ref cameraData, passColorAttachment, passDepthAttachment, finalClearFlag, finalClearColor,
