...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    internal class GBufferPass : ScriptableRenderPass
    {
        ...
        public override void Configure(CommandBuffer cmd, RenderTextureDescriptor cameraTextureDescriptor)
        {
            ...
            GraphicsFormat[] gbufferFormatsUsed = new GraphicsFormat[gbufferAttachments.Length];

            // Create and declare the render targets used in the pass
            for (int i = 0; i < gbufferAttachments.Length; ++i)
            {
                gbufferFormatsUsed[i] = m_DeferredLights.GetGBufferFormat(i);
                ...
                gbufferSlice.graphicsFormat = gbufferFormatsUsed[i];
                cmd.GetTemporaryRT(m_DeferredLights.GbufferAttachments[i].id, gbufferSlice);
            }

            srpDescriptor.ConfigureDescriptor(gbufferFormatsUsed);
