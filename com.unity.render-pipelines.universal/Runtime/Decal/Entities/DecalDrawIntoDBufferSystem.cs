...
public class DecalDrawIntoDBufferSystem
{
    ...
    public DecalDrawIntoDBufferSystem(DecalEntityManager entityManager)
    {
        ...
        m_Sampler = new ProfilingSampler("DecalDrawIntoDBufferSystem.Execute");
    }
    ...
    private void Execute(DecalEntityChunk decalEntityChunk, DecalCachedChunk decalCachedChunk, DecalDrawCallChunk decalDrawCallChunk, int count, CommandBuffer cmd)
    {
        ...
        for (int i = 0; i < subCallCount; ++i)
        {
            ...
            cmd.DrawMeshInstanced(m_DecalMesh, 0, decalEntityChunk.material, decalCachedChunk.passIndex, m_WorldToDecals, subCall.end - subCall.start, decalCachedChunk.propertyBlock);
        }
    }
}
