            ...
            using (new ProfilingScope(cmdBuffer, m_ProfilingSamplerShadows))
            {
                ...
                if (shadowCasterGroups != null && shadowCasterGroups.Count > 0)
                {
                    ...
                    if (hasShadow)
                    {
                        ...
                        {
                            if (colorBit == 0)
                                cmdBuffer.ClearRenderTarget(RTClearFlags.ColorStencil, Color.clear, 0, 0);  // clear stencil
                            else
                                cmdBuffer.ClearRenderTarget(RTClearFlags.Stencil, Color.clear, 0, 0); // clear stencil only 
