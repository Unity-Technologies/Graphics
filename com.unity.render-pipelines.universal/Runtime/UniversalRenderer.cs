    ...
    public sealed class UniversalRenderer : ScriptableRenderer
    {
        ...
        public override void Setup(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            if ((this.renderingModeActual == RenderingMode.Deferred && !this.useRenderPassEnabled) || requiresDepthPrepass || requiresDepthCopyPass)
            {
                ...
                RenderingUtils.ReAllocateIfNeeded(ref m_DepthTexture, Vector2.one, depthDescriptor, FilterMode.Point, wrapMode: TextureWrapMode.Clamp, name: "_CameraDepthTexture");
            ...
            if (requiresDepthPrepass && renderPassInputs.requiresNormalsTexture)
            {
                ...
                RenderingUtils.ReAllocateIfNeeded(ref normalsTexture, Vector2.one, normalDescriptor, FilterMode.Point, TextureWrapMode.Clamp, name: normalsTextureName);
            ...
            if (copyColorPass)
            {
                ...
                RenderingUtils.ReAllocateIfNeeded(ref m_OpaqueColor, Vector2.one, descriptor, filterMode, TextureWrapMode.Clamp, name: "_CameraOpaqueTexture");
            ...
            if (renderPassInputs.requiresMotionVectors && !cameraData.xr.enabled)
            {
                ...
                RenderingUtils.ReAllocateIfNeeded(ref m_MotionVectorColor, Vector2.one, colorDesc, FilterMode.Point, TextureWrapMode.Clamp, name: "_MotionVectorTexture");

                var depthDescriptor = cameraTargetDescriptor;
                depthDescriptor.graphicsFormat = GraphicsFormat.None;
                RenderingUtils.ReAllocateIfNeeded(ref m_MotionVectorDepth, Vector2.one, depthDescriptor, FilterMode.Point, TextureWrapMode.Clamp, name: "_MotionVectorDepthTexture");
            ...
            if (applyPostProcessing)
            {
                ...
                RenderingUtils.ReAllocateIfNeeded(ref m_PostProcessPasses.m_AfterPostProcessColor, Vector2.one, desc, FilterMode.Point, TextureWrapMode.Clamp, name: "_AfterPostProcessTexture");
            }
        ...
        void CreateCameraRenderTarget(ScriptableRenderContext context, ref RenderTextureDescriptor descriptor, bool primedDepth, CommandBuffer cmd)
        {
            using (new ProfilingScope(null, Profiling.createCameraRenderTarget))
            {
                ...
                if (m_CameraDepthAttachment == null || m_CameraDepthAttachment.nameID != BuiltinRenderTextureType.CameraTarget)
                {
                    ...
                    depthDescriptor.graphicsFormat = GraphicsFormat.None;
                    depthDescriptor.depthStencilFormat = k_DepthStencilFormat;
                    RenderingUtils.ReAllocateIfNeeded(ref m_CameraDepthAttachment, Vector2.one, depthDescriptor, FilterMode.Point, TextureWrapMode.Clamp, name: "_CameraDepthAttachment");
