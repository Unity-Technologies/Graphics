...
namespace UnityEditor.Rendering.Universal.ShaderGraph
{
    ...
    {
        static readonly GUID kSourceCodeGuid = new GUID("54980e1ec1f73d74ea96cd5f30ccae90"); // UniversalTerrainLitSubTarget.cs
        ...
        private void SetupTerrainDependencyShaders(ref TargetSetupContext context)
        {
            context.AddShaderDependency("AddPassShader", "Hidden/Universal Render Pipeline/Terrain/Lit (Add Pass)");
            context.AddShaderDependency("BaseMapShader", "Hidden/Universal Render Pipeline/Terrain/Lit (Base Pass)");
            context.AddShaderDependency("BaseMapGenShader", "Hidden/Universal Render Pipeline/Terrain/Lit (Basemap Gen)");
        }
        ...
        public override void Setup(ref TargetSetupContext context)
        {
            ...
            //context.AddGlobalPragma(Pragma.MultiCompileInstancing);
            //context.AddGlobalPragma(new PragmaDescriptor { value = $"instancing_options assumeuniformscaling nomatrices nolightprobe nolightmap" });

            //var universalRPType = typeof(UnityEngine.Rendering.Universal.UniversalRenderPipelineAsset);
            //if (!context.HasCustomEditorForRenderPipeline(universalRPType))
            //    context.AddCustomEditorForRenderPipeline(typeof(TerrainLitShaderGUI).FullName, universalRPType);
            ...
            SetupTerrainDependencyShaders(ref context);
        }
        ...
        #region pragmas
        static class TerrainLitPragmas
        {
        ...
        {
            private static string[] pickingPass =
            {
                "Hidden/Nature/Terrain/Utilities/PICKING"
            };
            private static string terrainTags = " \"TerrainCompatible\" = \"True\" \"IgnoreProjector\" = \"False\"";
            ...
            public static SubShaderDescriptor LitComputeDotsSubShader(UniversalTarget target, WorkflowMode workflowMode, string renderType, string renderQueue, bool complexLit)
            {
                SubShaderDescriptor result = new SubShaderDescriptor()
                {
                    ...
                    customTags = UniversalTarget.kLitMaterialTypeTag + terrainTags,
                    renderType = renderType,
                    renderQueue = renderQueue,
                    generatesPreview = true,
                    passes = new PassCollection(),
                    usePasses = new List<string>(pickingPass)
                };

                if (complexLit)
                    result.passes.Add(LitPasses.ForwardOnly(target, workflowMode, complexLit, CoreBlockMasks.Vertex, LitBlockMasks.FragmentComplexLit, TerrainLitPragmas.DOTSForward));
                else
                    result.passes.Add(LitPasses.Forward(target, workflowMode, TerrainLitPragmas.DOTSForward));
                ...
                if (target.castShadows || target.allowMaterialOverride)
                    result.passes.Add(PassVariant(TerrainLitPasses.ShadowCaster(target), TerrainLitPragmas.DOTSInstanced));

                if (target.mayWriteDepth)
                    result.passes.Add(PassVariant(CorePasses.DepthOnly(target), TerrainLitPragmas.DOTSInstanced));

                if (complexLit)
                    result.passes.Add(PassVariant(LitPasses.DepthNormalOnly(target), TerrainLitPragmas.DOTSInstanced));
                else
                    result.passes.Add(PassVariant(LitPasses.DepthNormal(target), TerrainLitPragmas.DOTSInstanced));
                ...
                result.passes.Add(PassVariant(TerrainLitPasses.SceneSelection(target), TerrainLitPragmas.DOTSInstanced));
            ...
            public static SubShaderDescriptor LitGLESSubShader(UniversalTarget target, WorkflowMode workflowMode, string renderType, string renderQueue, bool complexLit)
            {
                ...
                if (complexLit)
                    result.passes.Add(LitPasses.ForwardOnly(target, workflowMode, complexLit, CoreBlockMasks.Vertex, LitBlockMasks.FragmentComplexLit, TerrainLitPragmas.Forward));
                else
        ...
        static class TerrainLitPasses
        {
            ...
            {
                var result = CorePasses.ShadowCaster(target);
                result.pragmas = TerrainLitPragmas.Instanced;
                return result;
            }

            public static PassDescriptor SceneSelection(UniversalTarget target)
            {
                var result = CorePasses.SceneSelection(target);
                result.pragmas = TerrainLitPragmas.Instanced;
                return result;
            }
        }
        static class LitPasses
        {
            ...
            public static PassDescriptor Forward(UniversalTarget target, WorkflowMode workflowMode, PragmaCollection pragmas = null)
            {
                var result = new PassDescriptor()
                {
                    ...
                    pragmas = pragmas ?? TerrainLitPragmas.Forward,     // NOTE: SM 2.0 only GL
            ...
            public static PassDescriptor GBuffer(UniversalTarget target, WorkflowMode workflowMode)
            {
                var result = new PassDescriptor
                {
                    ...
                    // Conditional State
                    renderStates = CoreRenderStates.UberSwitchedRenderState(target),
                    pragmas = TerrainLitPragmas.DOTSGBuffer,
                    defines = new DefineCollection() { CoreDefines.UseFragmentFog },
