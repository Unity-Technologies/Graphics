...
| __Store Actions__          | Defines the policy used when setting store actions for the DrawObjects passes. It has a great impact on bandwidth on mobile and TBDR GPUs. <br/>__Auto__: Behaves like the Discard option. If any injected passes are detected will then fall back to Store. <br/>__Discard__: Targets of passes which are not re-used later in the frame will be discarded (optimal). <br/>__Store__: Store all the targets of each pass (unoptimal). |
