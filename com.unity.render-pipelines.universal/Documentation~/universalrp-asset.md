...
|__Light Layers__| Enable [Light Layers](https://docs.unity3d.com/Manual/light-layers.html) to make the render pipeline use the rendering layers instead of the Culling Mask property in Lights (Rendering > Culling Mask). This is typically used to determine how lights affect groups of geometry.
...
| __Volume Updater__ | Select how Unity updates volumes. The options are __Every Frame__ and __Via Scripting__. In the Editor, Unity updates volumes every frame when not in Play mode.
