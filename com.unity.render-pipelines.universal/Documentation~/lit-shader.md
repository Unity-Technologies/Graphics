...
| __Clear Coat Map__          | Adds a second material layer which simulates a transparent and thin coating on top of the base material. The feature is enabled by checking the Clear Coat checkbox. Setting the mask to 0 does not disable the feature. Clear coat has roughly twice the cost of a base material as lighting is evaluated once per layer. Clear coat supports mask (or intensity) and smoothness features. It's IOR is fixed to 1.5. The clear coat can be controlled using a map where the red channel represents the mask property and the green channel represents the smoothness property. If a map is provided then the constant values for mask and smoothness are used as a multiplier for the map's per pixel values. The clear coat slightly affects the color and the smoothness values of the underlying base material. Clear coat support on OpenGL ES is restricted to 3.1 and up.|
