...
half4 LitPassFragmentGrass(GrassVertexOutput input) : SV_Target
{
    ...
    float2 uv = input.uv;
    half4 diffuseAlpha = SampleAlbedoAlpha(uv, _MainTex, sampler_MainTex);
    ...
    half3 emission = 0;
    half4 specularGloss = 0.1;// SampleSpecularSmoothness(uv, diffuseAlpha.a, _SpecColor, _SpecGlossMap, sampler_SpecGlossMap);
...
{
    Alpha(SampleAlbedoAlpha(IN.uv, _MainTex, sampler_MainTex).a, IN.color, _Cutoff);
return 0;
}
