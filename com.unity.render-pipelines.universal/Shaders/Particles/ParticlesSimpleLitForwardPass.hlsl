...
half4 ParticlesLitFragment(VaryingsParticle input) : SV_Target
{
    ...
    half3 normalTS = SampleNormalTS(uv, blendUv, _BumpMap, sampler_BumpMap);
    half4 albedo = SampleAlbedo(uv, blendUv, _BaseColor, input.color, projectedPosition, _BaseMap, sampler_BaseMap);
    half3 diffuse = AlphaModulate(albedo.rgb, albedo.a);
    half alpha = albedo.a;
#if defined(_EMISSION)
    half3 emission = BlendTexture(_EmissionMap, sampler_EmissionMap, uv, blendUv) * _EmissionColor.rgb;
#else
    half3 emission = half3(0, 0, 0);
#endif
    half4 specularGloss = SampleSpecularSmoothness(uv, blendUv, albedo.a, _SpecColor, _SpecGlossMap, sampler_SpecGlossMap);
