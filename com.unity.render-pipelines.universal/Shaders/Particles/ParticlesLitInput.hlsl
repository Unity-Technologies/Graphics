...
inline void InitializeParticleLitSurfaceData(float2 uv, float3 blendUv, float4 particleColor, float4 projectedPosition, out SurfaceData outSurfaceData)
{
    ...
    half4 albedo = SampleAlbedo(uv, blendUv, _BaseColor, particleColor, projectedPosition, _BaseMap, sampler_BaseMap);

#if defined(_METALLICSPECGLOSSMAP)
    half2 metallicGloss = BlendTexture(_MetallicGlossMap, sampler_MetallicGlossMap, uv, blendUv).ra * half2(1.0, _Smoothness);
#else
    half2 metallicGloss = half2(_Metallic, _Smoothness);
#endif

    half3 normalTS = SampleNormalTS(uv, blendUv, _BumpMap, sampler_BumpMap, _BumpScale);
