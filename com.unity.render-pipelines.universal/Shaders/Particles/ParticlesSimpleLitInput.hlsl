...
half4 SampleAlbedo(float2 uv, float3 blendUv, half4 color, float4 particleColor, float4 projectedPosition, TEXTURE2D(albedoMap), SAMPLER(sampler_albedoMap))
{
    half4 albedo = BlendTexture(albedoMap, sampler_albedoMap, uv, blendUv) * color;
...
half4 SampleSpecularSmoothness(float2 uv, float3 blendUv, half alpha, half4 specColor, TEXTURE2D(specGlossMap), SAMPLER(sampler_specGlossMap))
{
    half4 specularGloss = half4(0.0h, 0.0h, 0.0h, 1.0h);
#ifdef _SPECGLOSSMAP
    specularGloss = BlendTexture(specGlossMap, sampler_specGlossMap, uv, blendUv);
