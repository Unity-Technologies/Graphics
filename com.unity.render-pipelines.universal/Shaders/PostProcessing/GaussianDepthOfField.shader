Shader "Hidden/Universal Render Pipeline/GaussianDepthOfField"
{
    HLSLINCLUDE
        ...
        PrefilterOutput FragPrefilter(Varyings input)
        {
            ...
            {
                float2 tapCoord = GetSourceSize().zw * kTaps[i] + uv;
        ...
        half4 Blur(Varyings input, float2 dir, float premultiply)
        {
            ...
            int2 positionSS = int2(GetSourceSize().xy * _DownSampleScaleFactor.xy * uv);
            half samp0CoC = LOAD_TEXTURE2D_X(_HalfCoCTexture, positionSS).x;

            float2 offset = GetSourceSize().zw * _DownSampleScaleFactor.zw * dir * samp0CoC * MaxRadius;
            half4 acc = 0.0;
        ...
        half4 FragComposite(Varyings input) : SV_Target
        {
            ...
            half3 baseColor = LOAD_TEXTURE2D_X(_SourceTex, GetSourceSize().xy * uv).xyz;
            half coc = LOAD_TEXTURE2D_X(_FullCoCTexture, GetSourceSize().xy * uv).x;

        #if _HIGH_QUALITY_SAMPLING && !defined(SHADER_API_GLES)
            half3 farColor = SampleTexture2DBicubic(TEXTURE2D_X_ARGS(_ColorTexture, sampler_LinearClamp), uv, GetSourceSize() * _DownSampleScaleFactor, 1.0, unity_StereoEyeIndex).xyz;
        #else
