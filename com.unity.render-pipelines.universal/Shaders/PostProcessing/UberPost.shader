    ...
    HLSLINCLUDE
        ...
        half4 Frag(Varyings input) : SV_Target
        {
            ...
            #if _CHROMATIC_ABERRATION
            {
                ...
                half r = SAMPLE_TEXTURE2D_X(_SourceTex, sampler_LinearClamp, CLUSTER_TO_DEVICE_FULLSCREEN_UV(uvDistorted)                ).x;
                half g = SAMPLE_TEXTURE2D_X(_SourceTex, sampler_LinearClamp, CLUSTER_TO_DEVICE_FULLSCREEN_UV(DistortUV(delta + uv)      )).y;
                half b = SAMPLE_TEXTURE2D_X(_SourceTex, sampler_LinearClamp, CLUSTER_TO_DEVICE_FULLSCREEN_UV(DistortUV(delta * 2.0 + uv))).z;

                color = half3(r, g, b);
            }
            #else
            {
                color = SAMPLE_TEXTURE2D_X(_SourceTex, sampler_LinearClamp, CLUSTER_TO_DEVICE_FULLSCREEN_UV(uvDistorted)).xyz;
            }
            #endif
