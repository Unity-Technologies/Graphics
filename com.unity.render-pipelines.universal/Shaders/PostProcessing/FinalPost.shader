Shader "Hidden/Universal Render Pipeline/FinalPost"
{
    HLSLINCLUDE
        ...
        {
            #if SHADER_API_GLES
            float2 uv = (icoords + int2(idx, idy)) * GetSourceSize().zw;
            return SAMPLE_TEXTURE2D_X(_SourceTex, sampler_LinearClamp, uv).xyz;
            #else
            return LOAD_TEXTURE2D_X(_SourceTex, clamp(icoords + int2(idx, idy), 0, GetSourceSize().xy - 1.0)).xyz;
            #endif
        }
        ...
        half4 Frag(Varyings input) : SV_Target
        {
            ...
            int2   positionSS  = uv * GetSourceSize().xy;
            ...
            #if _FXAA
            {
                ...
                dir = min((FXAA_SPAN_MAX).xx, max((-FXAA_SPAN_MAX).xx, dir * rcpDirMin)) * GetSourceSize().zw;
