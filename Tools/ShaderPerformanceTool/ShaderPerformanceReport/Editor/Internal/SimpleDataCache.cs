...
namespace UnityEditor.Experimental.ShaderTools.Internal
{
    class SimpleDataCache
    {
        ...
        {
            Item t;
            return m_Items.TryGetValue(key, out t) ? t.@byte : default(byte);
        }

        public short GetShort(int key)
        {
            Item t;
            return m_Items.TryGetValue(key, out t) ? t.@short : default(short);
        }

        public ushort GetUShort(int key)
        {
            Item t;
            return m_Items.TryGetValue(key, out t) ? t.@ushort : default(ushort);
        }

        public int GetInt(int key)
        {
            Item t;
            return m_Items.TryGetValue(key, out t) ? t.@int : default(int);
        }

        public uint GetUInt(int key)
        {
            Item t;
            return m_Items.TryGetValue(key, out t) ? t.@uint : default(uint);
        }

        public string GetString(int key)
        {
            Item t;
            return m_Items.TryGetValue(key, out t) ? t.@string : default(string);
        }
