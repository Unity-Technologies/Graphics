...
if __name__ == "__main__":
    ...
    gfx_sdet_tools_rev_file = os.path.join(
        current_yamato_dir, 'script', 'gfx_sdet_tools_revision.txt')

    with open(config_file) as f:
        config = yaml.load(f)
    gfx_sdet_tools_dir = config["gfx_sdet_tools_path"]
    build_py = os.path.join(
        gfx_sdet_tools_dir, 'yml-generator', 'ruamel', 'build.py')

    cmd = [sys.executable, build_py, '--yamato-dir', current_yamato_dir]
    print(f'Calling {cmd}')
