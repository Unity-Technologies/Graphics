...
def create_projectcontext_ymls(metafile):
    ...
    for package in metafile["packages"]:
        job = Project_PublishJob(package, metafile["agent_publish"], metafile["platforms"], metafile["publish_all_track"])
        yml[job.job_id] = job.yml

        job = Project_PublishJob_DryRun(package, metafile["agent_publish"], metafile["platforms"], metafile["publish_all_track"])
        yml[job.job_id] = job.yml
    ...
    for editor in metafile['editors']:
        job = Project_AllPackageCiJob(metafile["packages"], metafile["agent_publish"], metafile["platforms"], metafile["publish_all_track"], metafile["target_branch"], editor)
        yml[job.job_id] = job.yml
