...
from ..shared.namer import packages_filepath, package_job_id_test_all, package_job_id_test, package_job_id_test_dependencies, _track_name
...
class Package_AllPackageCiJob():
    ...
    def __init__(self, packages, agent, platforms, target_branch, editor):
        self.job_id = package_job_id_test_all(editor["track"], editor.get('fast'))
    ...
    def get_job_definition(self, packages, agent, platforms, target_branch, editor):
        ...
        for platform in platforms:
            for package in packages:
                dependencies.append(f'{packages_filepath()}#{package_job_id_test(package["id"],platform["os"],editor["track"], editor.get("fast"))}')
                #dependencies.append(f'{packages_filepath()}#{package_job_id_test_dependencies(package["id"],platform["os"],editor["track"])}')
        
        # construct job
        job = YMLJob()
        job.set_name(f'Pack and test all packages - { _track_name(editor["track"], editor.get("fast")) } [package context]')
        job.set_agent(agent)
