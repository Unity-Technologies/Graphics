...
class Project_ValidateJob():
    
    def __init__(self, platform, editor):
        self.job_id = projectcontext_job_id_validate(platform["os"],editor["version"])
    ...
    def get_job_definition(self, platform, editor):
        ...
        # construct job
        job = YMLJob()
        job.set_name(f'Validate all packages [project context] {platform["name"]} {editor["version"]}')
