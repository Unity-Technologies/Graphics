...
def cmd_standalone(project_folder, platform, api, test_platform_args):
    ...
    base.extend([
        f'cd {TEST_PROJECTS_DIR}/{project_folder} && utr\\utr --suite=playmode --platform=WSAPlayer --artifacts_path={PATH_TEST_RESULTS} --timeout=1200 --player-load-path=../../{PATH_PLAYERS} --player-connection-ip=auto'
    ])
    return base
...
def cmd_standalone_build(project_folder, platform, api, test_platform_args):
    base = _cmd_base(project_folder, platform["components"])
    base.extend([
        f'cd {TEST_PROJECTS_DIR}/{project_folder} && utr\\utr --suite=playmode --platform=WSAPlayer --extra-editor-arg="-executemethod" --extra-editor-arg="CustomBuild.BuildUWP{api}Linear" --testproject=. --editor-location=.Editor --artifacts_path={PATH_TEST_RESULTS} --timeout=1200 --player-save-path=../../{PATH_PLAYERS} --build-only'
    ])
    return base
