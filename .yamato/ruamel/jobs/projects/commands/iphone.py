...
def cmd_standalone(project_folder, platform, api, test_platform_args):
    return [
        ...
        f'chmod +x ./utr',
        f'./utr --suite=playmode --platform=iOS --player-load-path=players --artifacts_path=test-results'
    ]
...
def cmd_standalone_build(project_folder, platform, api, test_platform_args):
    ...
    return [
        ...
        f'curl -s https://artifactory.internal.unity3d.com/core-automation/tools/utr-standalone/utr --output utr',
        f'chmod +x ./utr',
        f'./utr --suite=playmode --platform=iOS --editor-location=.Editor --testproject={TEST_PROJECTS_DIR}/{project_folder} --player-save-path=players --artifacts_path={PATH_TEST_RESULTS} --build-only'
     ]