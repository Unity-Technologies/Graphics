...
def cmd_standalone_build(project, platform, api, test_platform_args):
    return  [
        ...
        f'cd {TEST_PROJECTS_DIR}/{project["folder"]} && unity-downloader-cli --source-file ../../{PATH_UNITY_REVISION} {"".join([f"-c {c} " for c in platform["components"]])} --wait --published-only',
