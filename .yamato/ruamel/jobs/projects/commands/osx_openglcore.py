...
def cmd_not_standalone(project, platform, api, test_platform_args):
    ...
    base.extend([ 
        f'cd TestProjects/{project["folder"]} && utr/utr {test_platform_args} --testproject=. --editor-location=.Editor --artifacts_path=test-results'
    ])
    return base
...
def cmd_standalone(project, platform, api, test_platform_args):
    base = _cmd_base(project, platform["components"])
    base.extend([
        f'cd TestProjects/{project["folder"]} && utr/utr {test_platform_args}OSX --testproject=. --editor-location=.Editor --artifacts_path=test-results --timeout=1200 --player-load-path=players --player-connection-ip=auto'
    ])
    return base
