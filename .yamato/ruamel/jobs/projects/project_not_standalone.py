...
class Project_NotStandaloneJob():
    ...
    def get_job_definition(self, project, editor, platform, api, test_platform, build_config, color_space):
        if 'URPUpdate' in project["name"]:
            ...
            job = _job(project, test_platform["name"], editor, platform, api, cmd(project["folder"], project["url"], project["branch"], project["revision"], project["dependencies"], platform, api, test_platform, editor, build_config, color_space), build_config, color_space)
            return job
        else:
