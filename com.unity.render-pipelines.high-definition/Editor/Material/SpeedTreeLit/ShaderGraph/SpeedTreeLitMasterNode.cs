...
namespace UnityEditor.Rendering.HighDefinition
{
    ...
    class SpeedTreeLitMasterNode : MasterNode<ISpeedTreeLitSubShader>, IMayRequirePosition, IMayRequireNormal, IMayRequireTangent
    {
        ...
        // In speedTree Alpha Test is always on, so there is no need to explicitly store it
        // or offer a control for it as we do for HDLit
        ...
        public sealed override void UpdateNodeAfterDeserialization()
        {
            ...
            if (MaterialTypeUsesSlotMask(SlotMask.AlphaThresholdDepthPrepass) && surfaceType == SurfaceType.Transparent && alphaTestDepthPrepass.isOn)
            {
            ...
            if (MaterialTypeUsesSlotMask(SlotMask.AlphaThresholdDepthPostpass) && surfaceType == SurfaceType.Transparent && alphaTestDepthPostpass.isOn)
            {
            ...
            if (MaterialTypeUsesSlotMask(SlotMask.AlphaThresholdShadow) && alphaTestShadow.isOn)
            {
        ...
        public override void ProcessPreviewMaterial(Material previewMaterial)
        {
            ...
            SpeedTreeLitGUI.SetupMaterialKeywordsAndPass(previewMaterial);
        }
