...
{
    static Texture2D GenerateWorleyfBm(int width, int height, int depth)
    {
        ...
        RenderTexture rTexture0 = new RenderTexture(width, height, 1, GraphicsFormat.R8G8B8A8_SNorm);
        ...
        int kernel = worleyCS.FindKernel("WorleyNoiseEvaluator");

        // Create the intermediate texture
        Texture2D tex2d = new Texture2D(width, height * depth, TextureFormat.RGBA32, false);
    ...
    {
        Texture2D result = GenerateWorleyfBm(128, 128, 128);
