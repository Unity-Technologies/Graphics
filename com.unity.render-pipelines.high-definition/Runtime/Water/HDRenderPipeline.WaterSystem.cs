...
namespace UnityEngine.Rendering.HighDefinition
{
    ...
    internal class WaterSiumulationResources
    {
        ...
        // Surface data that should force the spectrum re-evaluation
        public float m_WindSpeed = 0;
        public float m_WindOrientation = 0;
        public float m_WindAffectCurrent = 0;
        public float m_WaterMaxPatchSize = 0;
        ...
        public void AllocateSmmulationResources(int simulationRes, int numBands)
        {
            ...
            m_AdditionalData = RTHandles.Alloc(m_SimulationSize, m_SimulationSize, m_NumBands, dimension: TextureDimension.Tex2DArray, colorFormat: GraphicsFormat.R16G16B16A16_SFloat, enableRandomWrite: true, wrapMode: TextureWrapMode.Repeat, useMipMap: true, autoGenerateMips: false);
        }

        public bool ValidResources(int simulationRes, int numBands)
        {
            return (simulationRes == m_SimulationSize) && (numBands == m_NumBands) && (m_H0s != null) && (m_DisplacementBuffer != null) && (m_AdditionalData != null);
        }

        public bool AllocatedTextures()
        {
            return (m_H0s != null) || (m_DisplacementBuffer != null) || (m_AdditionalData != null);
        }
        ...
        public void ReleaseSmmulationResources()
        {
            ...
            // Simulation definition resolution
            m_SimulationSize = 0;
            m_NumBands = 0;
            m_Time = 0;
            m_SimulationTime = 0;
            ...
            m_WindSpeed = 0;
            m_WindOrientation = 0;
            m_WindAffectCurrent = 0;
            m_WaterMaxPatchSize = 0;
        }
    }
    ...
    public partial class HDRenderPipeline
    {
        ...
        const int k_WaterMinGridSize = 8;
        ...
        public float ComputeMaximumWaveHeight(Vector4 normalizedWaveAmplitude, float waterMaxPatchSize, float waterWindSpeed, int numBands)
        {
        ...
        float EvaluatePolynomial(float x, float k_4, float k_3, float k_2, float k_1, float k_0)
        {
            float x2 = x * x;
            float x3 = x2 * x;
            float x4 = x2 * x2;
        ...
        void UpdateShaderVariablesWater(WaterSurface currentWater, ref ShaderVariablesWater cb)
        {
            ...
            cb._BandPatchUVScale = new Vector4(1.0f, 7.0f, 31.0f, 63.0f);

            // Wind direction
            cb._WindDirection = OrientationToDirection(currentWater.windOrientation);
        
            // Wind speed per band
            float windMpS = currentWater.windSpeed * k_KilometerPerHourToMeterPerSecond;
            float maxWindForBand0 = MaximumWindForPatch(cb._BandPatchSize.x);
            cb._WindSpeed.x = windMpS;
            cb._WindSpeed.y = windMpS * MaximumWindForPatch(cb._BandPatchSize.y) / maxWindForBand0 * Mathf.Pow(0.7f, 0.333333f * 2);
            cb._WindSpeed.z = windMpS * MaximumWindForPatch(cb._BandPatchSize.z) / maxWindForBand0 * Mathf.Pow(0.7f, 0.333333f * 4);
                ...
                for (int surfaceIdx = 0; surfaceIdx < numWaterSurfaces; ++surfaceIdx)
                {
                    ...
                    if (!validResources)
                    {
                        ...
                        if (initialAllocation)
                            CoreUtils.SetRenderTarget(cmd, currentWater.simulation.m_AdditionalData, clearFlag: ClearFlag.Color, Color.black);
                    ...
                    cmd.SetComputeTextureParam(m_WaterSimulationCS, m_EvaluateNormalsKernel, HDShaderIDs._WaterDisplacementBuffer, currentWater.simulation.m_DisplacementBuffer);
                    ...
                    cmd.SetComputeTextureParam(m_WaterSimulationCS, m_BlurFoamAndCombineKernel, HDShaderIDs._WaterAdditionalDataBufferRW, currentWater.simulation.m_AdditionalData);
                    ...
                    currentWater.simulation.m_AdditionalData.rt.GenerateMips();
                }
            }
        }
        ...
        static float[] offsets = { 0.0f, 1.5f, 4.5f, 8.5f, 15.5f };
        ...
        void RenderWaterSurfaces(RenderGraph renderGraph, HDCamera hdCamera, TextureHandle colorBuffer, TextureHandle depthBuffer, TextureHandle colorPyramid)
        {
            ...
            for (int surfaceIdx = 0; surfaceIdx < numWaterSurfaces; ++surfaceIdx)
            {
                ...
                using (var builder = renderGraph.AddRenderPass<WaterRenderingData>("Render Water Surfaces", out var passData, ProfilingSampler.Get(HDProfileId.WaterSurfaceRendering)))
                {
                    ...
                    passData.additionalData = renderGraph.ImportTexture(currentWater.simulation.m_AdditionalData);
                    ...
                    builder.SetRenderFunc(
                        (WaterRenderingData data, RenderGraphContext ctx) =>
                        {
                            ...
                            CoreUtils.SetKeyword(ctx.cmd, "WATER_QUAD_GEOMETRY", data.parameters.infinite || data.parameters.targetMesh == null);
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    for (int subMeshIdx = 0; subMeshIdx < numSubMeshes; ++subMeshIdx)
                                        ctx.cmd.DrawMesh(data.parameters.targetMesh, Matrix4x4.identity, data.parameters.waterMaterial, subMeshIdx, 0, data.parameters.mbp);
                                }
                            }
                        });
                }
