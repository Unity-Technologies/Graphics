...
void GetSurfaceData(VFX_VARYING_PS_INPUTS input, float3 V, float2 posSS, float angleFadeFactor, out DecalSurfaceData surfaceData)
{
#if (SHADERPASS == SHADERPASS_VFX_DBUFFER_PROJECTOR) || (SHADERPASS == SHADERPASS_FORWARD_EMISSIVE_PROJECTOR)
    ...
    LODDitheringTransition(ComputeFadeMaskSeed(V, posSS), unity_LODFade.x);
    #endif
