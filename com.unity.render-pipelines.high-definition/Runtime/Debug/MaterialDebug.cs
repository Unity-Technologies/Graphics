...
namespace UnityEngine.Rendering.HighDefinition
{
    ...
    public class MaterialDebugSettings
    {
        ...
        // GBuffer material debug view enums are autogenerated, so we have to query the value
        internal static int GetGBufferMaterialEnumByName(string enumValue)
        {
            for (int i = 0; i < debugViewMaterialGBufferStrings.Length; i++)
            {
                if (debugViewMaterialGBufferStrings[i].text.Equals(enumValue))
                {
                    return debugViewMaterialGBufferValues[i];
                }
            }
            return -1;
        }
