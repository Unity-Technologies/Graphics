    ...
    public partial class HDRenderPipeline
    {
        ...
        private void BindAPVRuntimeResources(CommandBuffer cmdBuffer, HDCamera hdCamera)
        {
            ...
            if (hdCamera.frameSettings.IsEnabled(FrameSettingsField.ProbeVolume))
            {
                ...
                if (validResources)
                {
                    ...
                    cmdBuffer.SetGlobalTexture(HDShaderIDs._OctahedralDepthAtlas, rr.octaDepth);
        ...
        private void UpdateShaderVariablesProbeVolumes(ref ShaderVariablesGlobal cb, HDCamera hdCamera, CommandBuffer cmd)
        {
            ...
            if (cb._EnableProbeVolumes > 0)
            {
                ...
                parameters.leakParams = new Vector4((float)probeVolumeOptions.antiLeakMode.value, probeVolumeOptions.leakWeightContrib.value, probeVolumeOptions.oracleThresh.value, 1);
                parameters.leakParamsDBG = new Vector4(probeVolumeOptions.neighbourIndices.value, 0, 0, probeVolumeOptions.debug.value ? 1 : 0);
