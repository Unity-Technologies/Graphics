...
void GetSurfaceAndBuiltinData(FragInputs input, float3 V, inout PositionInputs posInput, out SurfaceData surfaceData, out BuiltinData builtinData RAY_TRACING_OPTIONAL_PARAMETERS)
{
    ...
    // custom change begin: added  * input.color
    float4 unlitColor = SAMPLE_TEXTURE2D(_UnlitColorMap, sampler_UnlitColorMap, unlitColorMapUv).rgba * _UnlitColor.rgba * input.color;
    // custom change end

    surfaceData.color = unlitColor.rgb;
    float alpha = unlitColor.a;
