...
namespace UnityEngine.Rendering.HighDefinition
{
    public partial class HDRenderPipeline
    {
        ...
        void WriteToScreenSpaceShadowBuffer(CommandBuffer cmd, HDCamera hdCamera, RTHandle source, int shadowSlot, bool colorShadow)
        {
            // Texture dimensions
            ...
            int shadowKernel = colorShadow ? m_OutputColorShadowTextureKernel : m_OutputShadowTextureKernel;

            // Bind the input data
            GetShadowChannelMask(shadowSlot, ScreenSpaceShadowType.GrayScale, ref m_ShadowChannelMask0);
            ...
            if (m_CurrentSunLightAdditionalLightData != null && m_CurrentSunLightAdditionalLightData.WillRenderScreenSpaceShadow())
            {
                ...
                if (m_CurrentSunLightAdditionalLightData.WillRenderRayTracedShadow())
                {
                    using (new ProfilingSample(cmd, "Directional Light Ray Traced Shadow", CustomSamplerId.RaytracingDirectionalLightShadow.GetSampler()))
                    {
                        ...
                        WriteToScreenSpaceShadowBuffer(cmd, hdCamera, intermediateBuffer0, dirShadowIndex, m_CurrentSunLightAdditionalLightData.colorShadow);
                    }
                }
                else
                {
        ...
        bool RenderLightScreenSpaceShadows(HDCamera hdCamera, CommandBuffer cmd)
        {
                ...
                , RTHandle shadowHistoryArray, RTHandle shadowHistoryValidityArray)
            {
                ...
                if (additionalLightData.filterTracedShadow)
                {
                    ...
                    // Write the result texture to the screen space shadow buffer
                    WriteToScreenSpaceShadowBuffer(cmd, hdCamera, intermediateBufferRGBA0, areaShadowSlot, false);
                }
                else
                {
                    int areaShadowSlot = lightData.screenSpaceShadowIndex;
                    GetShadowChannelMask(areaShadowSlot, ScreenSpaceShadowType.Area, ref m_ShadowChannelMask0);
                    cmd.SetComputeVectorParam(m_ScreenSpaceShadowsCS, HDShaderIDs._DenoisingHistoryMask, m_ShadowChannelMask0);
                    cmd.SetComputeIntParam(m_ScreenSpaceShadowsFilterCS, HDShaderIDs._DenoisingHistorySlice, areaShadowSlot / 4);
                ...
                , RTHandle shadowHistoryArray, RTHandle shadowHistoryValidityArray)
            {
                ...
                // Write the result texture to the screen space shadow buffer
                WriteToScreenSpaceShadowBuffer(cmd, hdCamera, intermediateBuffer0, lightData.screenSpaceShadowIndex, false);
        }
        ...
        void EvaluateShadowDebugView(CommandBuffer cmd, HDCamera hdCamera)
        {
            ...
            if (FullScreenDebugMode.ScreenSpaceShadows == hdrp.m_CurrentDebugDisplaySettings.data.fullScreenDebugMode)
            {
                ...
                if (m_ScreenSpaceShadowChannelSlot > hdrp.m_CurrentDebugDisplaySettings.data.screenSpaceShadowIndex)
                {
                    ...
                    cmd.SetComputeIntParam(shadowFilter, HDShaderIDs._DenoisingHistorySlice, (int)hdrp.m_CurrentDebugDisplaySettings.data.screenSpaceShadowIndex);
