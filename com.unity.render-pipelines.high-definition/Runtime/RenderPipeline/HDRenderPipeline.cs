...
// If we are in the editor, we have to take the current target build platform as the graphic device type is always the same
        ...
        public override void Render(ScriptableRenderContext renderContext, Camera[] cameras)
        {
            ...
            for (int cameraIndex = 0; cameraIndex < cameras.Length; ++cameraIndex)
            {
                ...
                using (new ProfilingSample(cmd, "HDRenderPipeline::Render", CustomSamplerId.HDRenderPipelineRender.GetSampler()))
                {
                    ...
                    {
                        if (m_UseNewCulling)
                            enableBakeShadowMask = m_LightLoop.PrepareLightsForGPU(cmd, hdCamera, m_CullResults, m_LightCullingResult, m_ReflectionProbeCullingResult, m_ReflectionProbeCullResults, densityVolumes, m_DebugDisplaySettings);
                        else
                            enableBakeShadowMask = m_LightLoop.PrepareLightsForGPU(cmd, hdCamera, m_CullResults, null, null, m_ReflectionProbeCullResults, densityVolumes, m_DebugDisplaySettings);
                    }
            ...
            Material overrideMaterial = null)
        {
            ...
            RenderOpaqueRenderList(cull, hdCamera, renderContext, cmd, m_SinglePassName, rendererList, rendererConfiguration, inRenderQueueRange, stateBlock, overrideMaterial);
        }
        ...
        {
            m_SinglePassName[0] = passName;
            RenderTransparentRenderList(cull, hdCamera, renderContext, cmd, m_SinglePassName, rendererList,
        ...
        void AccumulateDistortion(CullResults cullResults, HDCamera hdCamera, ScriptableRenderContext renderContext, CommandBuffer cmd)
        {
            ...
            using (new ProfilingSample(cmd, "Distortion", CustomSamplerId.Distortion.GetSampler()))
            {
                ...
                RenderTransparentRenderList(cullResults, hdCamera, renderContext, cmd, HDShaderPassNames.s_DistortionVectorsName, m_RendererLists[(int)HDRendererList.Distortion]);
            }
        }
        ...
        void RenderDepthPrepass(CullResults cull, HDCamera hdCamera, ScriptableRenderContext renderContext, CommandBuffer cmd)
        {
            ...
            else // Deferred with partial depth prepass
            {
                using (new ProfilingSample(cmd, "Depth Prepass (deferred incomplete)", CustomSamplerId.DepthPrepass.GetSampler()))
                {
                    ...
                    RenderOpaqueRenderList(cull, hdCamera, renderContext, cmd, m_DepthOnlyPassNames, m_RendererLists[(int)HDRendererList.DepthAlphaTest], 0, renderQueueRange, excludeMotionVector : excludeMotion);
        ...
        void RenderGBuffer(CullResults cull, HDCamera hdCamera, ScriptableRenderContext renderContext, CommandBuffer cmd)
        {
            ...
            using (new ProfilingSample(cmd, m_CurrentDebugDisplaySettings.IsDebugDisplayEnabled() ? "GBuffer Debug" : "GBuffer", CustomSamplerId.GBuffer.GetSampler()))
            {
                ...
                RenderOpaqueRenderList(cull, hdCamera, renderContext, cmd, HDShaderPassNames.s_GBufferName, m_RendererLists[(int)HDRendererList.GBuffer], m_currentRendererConfigurationBakedLighting, HDRenderQueue.k_RenderQueue_AllOpaque);
        ...
        void RenderDebugViewMaterial(CullResults cull, HDCamera hdCamera, ScriptableRenderContext renderContext, CommandBuffer cmd)
        {
            using (new ProfilingSample(cmd, "DisplayDebug ViewMaterial", CustomSamplerId.DisplayDebugViewMaterial.GetSampler()))
            {
                ...
                else
                {
                    ...
                    RenderOpaqueRenderList(cull, hdCamera, renderContext, cmd, m_AllForwardOpaquePassNames, m_RendererLists[(int)HDRendererList.OpaqueDebug], m_currentRendererConfigurationBakedLighting, stateBlock: m_DepthStateOpaque);

                    // Render forward transparent
                    RenderTransparentRenderList(cull, hdCamera, renderContext, cmd, m_AllTransparentPassNames, m_RendererLists[(int)HDRendererList.TransparentDebug], m_currentRendererConfigurationBakedLighting, stateBlock: m_DepthStateOpaque);
                }
            }
        ...
        void RenderTransparentDepthPrepass(CullResults cull, HDCamera hdCamera, ScriptableRenderContext renderContext, CommandBuffer cmd)
        {
            if (hdCamera.frameSettings.enableTransparentPrepass)
            {
                ...
                {
                    RenderTransparentRenderList(cull, hdCamera, renderContext, cmd, m_TransparentDepthPrepassNames, m_RendererLists[(int)HDRendererList.TransparentDepthPrepass]);
                }
            }
        }
        ...
        void RenderTransparentDepthPostpass(CullResults cullResults, HDCamera hdCamera, ScriptableRenderContext renderContext, CommandBuffer cmd)
        {
            ...
            {
                HDUtils.SetRenderTarget(cmd, hdCamera, m_SharedRTManager.GetDepthStencilBuffer());
                RenderTransparentRenderList(cullResults, hdCamera, renderContext, cmd, m_TransparentDepthPostpassNames, m_RendererLists[(int)HDRendererList.ForwardTransparentPostPass]);
            }
        }
        ...
        void RenderObjectsVelocity(CullResults cullResults, HDCamera hdCamera, ScriptableRenderContext renderContext, CommandBuffer cmd)
        {
            ...
            using (new ProfilingSample(cmd, "Objects Velocity", CustomSamplerId.ObjectsVelocity.GetSampler()))
            {
                ...
                RenderOpaqueRenderList(cullResults, hdCamera, renderContext, cmd, HDShaderPassNames.s_MotionVectorsName, m_RendererLists[(int)HDRendererList.ObjectVelocity], RendererConfiguration.PerObjectMotionVectors);
            }
        }
