...
namespace UnityEngine.Rendering
{
    static class ImportanceSampler2D
    {
                                ...
                                bool dumpFile, int idx)
        {
            ...
            //invCDFRows = RTHandles.Alloc(1,     height, colorFormat: format1, autoGenerateMips: false, enableRandomWrite: true, useMipMap: false);
            //invCDFFull = RTHandles.Alloc(width, height, colorFormat: format4, autoGenerateMips: false, enableRandomWrite: true, useMipMap: false);
            ...
            Rescale(sumRows, minMaxSumRows, GPUOperation.Direction.Vertical, cmd, true);
            if (dumpFile)
                cmd.RequestAsyncReadback(sumRows, delegate (AsyncGPUReadbackRequest request)
                {
            ...
            // 7. Rescale CDF Rows
            Rescale(cdfFull, minMaxRows, GPUOperation.Direction.Horizontal, cmd);
            if (dumpFile)
                cmd.RequestAsyncReadback(cdfFull, delegate (AsyncGPUReadbackRequest request)
                {
            ...
            // 8. Rescale CDF L
            Rescale(cdfL, minMaxCDFL, GPUOperation.Direction.Vertical, cmd, true);
            if (dumpFile)
                cmd.RequestAsyncReadback(cdfL, delegate (AsyncGPUReadbackRequest request)
                {
            ...
            // 9. Inv CDF Rows
            invCDFFull = ComputeCDF1D.ComputeInverseCDF(cdfFull, pdfCopy, cmd, ComputeCDF1D.SumDirection.Horizontal, format4);
            GraphicsFormat formatfff = invCDFFull.rt.graphicsFormat;
            if (dumpFile)
                cmd.RequestAsyncReadback(invCDFFull, delegate (AsyncGPUReadbackRequest request)
                {
                    Default(request, "09_InvCDFRows" + strName, formatfff);
                });
            // 10. Inv CDF L
            invCDFRows = ComputeCDF1D.ComputeInverseCDF(cdfL,    pdfCopy, cmd, ComputeCDF1D.SumDirection.Vertical, format1);
            GraphicsFormat formatffff = invCDFRows.rt.graphicsFormat;
            if (dumpFile)
                cmd.RequestAsyncReadback(invCDFRows, delegate (AsyncGPUReadbackRequest request)
                {
                    Default(request, "10_InvCDFL" + strName, formatffff);
                });
            // 11. Generate Samples
            uint samplesCount = 4096;
            RTHandle samples = GenerateSamples(samplesCount, invCDFRows, invCDFFull, GPUOperation.Direction.Horizontal, cmd, true);
        ...
        public static RTHandle GenerateSamples(uint samplesCount, RTHandle sliceInvCDF, RTHandle fullInvCDF, GPUOperation.Direction direction, CommandBuffer cmd, bool hemiSphere = false)
        {
            var hdrp = HDRenderPipeline.defaultAsset;
            ...
            if (cmd != null)
            {
                ...
                if (direction == GPUOperation.Direction.Horizontal)
                    cmd.SetComputeFloatParams(importanceSample2D,         HDShaderIDs._Sizes,
                                                fullInvCDF.rt.height, 1.0f/fullInvCDF.rt.height, 0.5f/fullInvCDF.rt.height, samplesCount);
                else
                    cmd.SetComputeFloatParams(importanceSample2D, HDShaderIDs._Sizes,
                                                fullInvCDF.rt.width, 1.0f/fullInvCDF.rt.width, 0.5f/fullInvCDF.rt.width, samplesCount);
