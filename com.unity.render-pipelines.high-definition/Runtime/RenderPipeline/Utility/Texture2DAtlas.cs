...
namespace UnityEngine.Rendering.HighDefinition
{
    ...
    class Texture2DAtlas
    {
        ...
        {
            // In the case of RTs, the value stored in m_IsGPUTextureUpToDate can be any (possibly) large int value - it's just storing the update count.
            // In the case of regular textures, we simply store 2 if mip maps were generated by the last update, or 1 if they were not.
            // We need to very carefully handle both cases here.
            // TODO: Would be better to seperate the concept of the update timestamp, and whether or not that update generated mips into 2 seperate variables
            // that we can actually reason about.
            // This would require updating all the blit functions to take in another optional argument: updateCount
            // Rather than updating all the APIs right now, I'm simply going to look at the value and determine which case it was, this is imperfect but will catch the vast majority of cases.
        ...
        private int EncodeGPUTextureUpToDateData(int updateCount, int flags)
        {
        ...
        public virtual bool NeedsUpdate(Texture texture, bool needMips = false)
        {
            ...
            if (rt != null)
            {
                ...
                if (m_IsGPUTextureUpToDate.TryGetValue(key, out updateCountAndFlags))
                {
                    ...
                    if ((rt.updateCount != updateCount) || (needMips && !mipsAlreadyGenerated))
                    {
                        // custom-begin:
