...
namespace UnityEngine.Rendering.HighDefinition
{
    ...
    public class HDCameraInfo
    {
        ...
        internal Matrix4x4 nonObliqueProjMatrix
        {
            get
            {
                return m_HDCamera != null
                    ? m_HDCamera.GetNonObliqueProjection(camera)
        ...
        {
            get { return m_HDCamera != null ? m_HDCamera.clearDepth : camera.clearFlags != CameraClearFlags.Nothing; }
        }

        internal HDCamera.ClearColorMode clearColorMode
        {
            get
            {
                if (m_HDCamera != null)
                {
                    return m_HDCamera.clearColorMode;
                }
        ...
        internal Color backgroundColorHDR
        {
            get
            {
                if (m_HDCamera != null)
                {
                    return m_HDCamera.backgroundColorHDR;
                }
        ...
        internal HDCamera.FlipYMode flipYMode
        {
            get
            {
                if (m_HDCamera != null)
                    return m_HDCamera.flipYMode;
        ...
        internal HDCamera.SMAAQualityLevel SMAAQuality { get; private set; } = HDCamera.SMAAQualityLevel.Medium;

        internal bool resetPostProcessingHistory = true;

        internal bool dithering => m_HDCamera != null && m_HDCamera.dithering;

        internal bool stopNaNs => m_HDCamera != null && m_HDCamera.stopNaNs;

        internal HDPhysicalCamera physicalParameters { get; private set; }

        internal IEnumerable<AOVRequestData> aovRequests =>
            m_HDCamera != null && !m_HDCamera.Equals(null)
                ? m_HDCamera.aovRequests
                : Enumerable.Empty<AOVRequestData>();

        internal LayerMask probeLayerMask
            => m_HDCamera != null
            ? m_HDCamera.probeLayerMask
            : (LayerMask)~0;

        internal float probeRangeCompressionFactor
            => m_HDCamera != null
            ? m_HDCamera.probeCustomFixedExposure
            : 1.0f;
        ...
        internal ProfilingSampler profilingSampler => m_HDCamera?.profilingSampler ?? ProfilingSampler.Get(HDProfileId.HDRenderPipelineRenderCamera);

        internal HDCameraInfo(Camera cam)
        {
            camera = cam;
        ...
        internal void Update(FrameSettings currentFrameSettings, HDRenderPipeline hdrp, MSAASamples newMSAASamples, XRPass xrPass)
        {
            ...
            camera.TryGetComponent<HDCamera>(out m_HDCamera);
        ...
        internal static void CleanUnused()
        {
            ...
            foreach (var key in s_Cameras.Keys)
            {
                ...
                bool hasPersistentHistory = camera.m_HDCamera != null && camera.m_HDCamera.hasPersistentHistory;
        ...
        HDCamera m_HDCamera = null; // Init in Update
...
#endif
                else if (m_HDCamera != null)
                {
                    antialiasing = m_HDCamera.antialiasing;
                    SMAAQuality = m_HDCamera.SMAAQuality;
                    taaSharpenStrength = m_HDCamera.taaSharpenStrength;
                }
                else
                    antialiasing = AntialiasingMode.None;
            }
        ...
        void UpdateVolumeAndPhysicalParameters()
        {
            ...
            if (m_HDCamera != null)
            {
                volumeLayerMask = m_HDCamera.volumeLayerMask;
                volumeAnchor = m_HDCamera.volumeAnchorOverride;
                physicalParameters = m_HDCamera.physicalParameters;
            }
            else
            {
                // Temporary hack:
                ...
                if (camera.cameraType == CameraType.SceneView)
                {
                    ...
                    if (mainCamera != null)
                    {
                        if (mainCamera.TryGetComponent<HDCamera>(out var mainCamAdditionalData))
                        {
