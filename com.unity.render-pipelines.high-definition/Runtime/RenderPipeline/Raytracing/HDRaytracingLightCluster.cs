...
namespace UnityEngine.Rendering.HighDefinition
{
    ...
    // Temporary variables
        ...
        void BuildGPULightVolumes(HDCamera hdCamera, HDRayTracingLights rayTracingLights)
        {
            ...
            int realIndex = 0;
            HDLightRenderDatabase lightEntites = HDLightRenderDatabase.instance;
            for (int lightIdx = 0; lightIdx < rayTracingLights.hdLightEntityArray.Count; ++lightIdx)
            {
                int dataIndex = lightEntites.GetEntityDataIndex(rayTracingLights.hdLightEntityArray[lightIdx]);
                HDAdditionalLightData currentLight = lightEntites.hdAdditionalLightData[dataIndex];
        ...
        void BuildLightData(CommandBuffer cmd, HDCamera hdCamera, HDRayTracingLights rayTracingLights, DebugDisplaySettings debugDisplaySettings)
        {
            ...
            for (int lightIdx = 0; lightIdx < rayTracingLights.hdLightEntityArray.Count; ++lightIdx)
            {
                ...
                processedLightEntity.distanceToCamera = (additionalLightData.gameObject.transform.position - hdCamera.camera.transform.position).magnitude;
