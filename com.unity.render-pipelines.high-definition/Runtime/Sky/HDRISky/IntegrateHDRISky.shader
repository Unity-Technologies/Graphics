Shader "Hidden/HDRP/IntegrateHDRI"
{
    ...
    real4 GetSphereLuxValue(TEXTURECUBE_PARAM(skybox, sampler_skybox))
    {
        float4 sum = 0.0;
        const float dphi    = 0.005;
        const float dtheta  = 0.005;
        ...
        for (float phi = 0; phi < 2.0*PI; phi += dphi)
        {
            for (float theta = 0; theta < PI; theta += dtheta)
            {
                float3 L = TransformGLtoDX(SphericalToCartesian(phi, cos(theta)));
                real3 val = SAMPLE_TEXTURECUBE_LOD(skybox, sampler_skybox, L, 0).rgb;
                sum += coef*sin(theta)*float4(val, max(val.r, max(val.g, val.b)));
            }
        }
    ...
    float4 FragHemi(Varyings input) : SV_Target
    {
    ...
    {
        float4 integral = GetSphereLuxValue(TEXTURECUBE_ARGS(_Cubemap, s_trilinear_clamp_sampler));
