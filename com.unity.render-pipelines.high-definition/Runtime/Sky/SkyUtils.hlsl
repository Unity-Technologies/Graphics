...
float3 GetSkyViewDirWS(float2 positionCS, float3x3 pixelCoordToViewDirWS)
{
    ...
    PositionInputs posInput = GetPositionInput_Stereo(positionCS, _ScreenSize.zw, UNITY_RAW_FAR_CLIP_VALUE, UNITY_MATRIX_I_VP, UNITY_MATRIX_V, unity_StereoEyeIndex);
    float3 viewDirWS = GetCurrentViewPosition() - posInput.positionWS;
#else
    // Points towards the camera
    float3 viewDirWS = mul(float3(positionCS, 1.0), pixelCoordToViewDirWS);
#endif
