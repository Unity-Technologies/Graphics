...
// last RT bind. Which maybe vary if we use 3 or 4 Render target or we are in debug mode
    ...
    // In debug mode though , u3 is used by the debug display
    #ifdef DEBUG_DISPLAY
        #ifdef DECALS_4RT
        RWStructuredBuffer<uint> _DecalPropertyMaskBuffer : register(u5);
        #else
