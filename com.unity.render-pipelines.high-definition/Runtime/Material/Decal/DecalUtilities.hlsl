...
if (g_prepasslessDecals != 0) {
    posInput.tileCoord = uint2(posInput.positionSS.xy) / DecalGetTileSize();
    ...
    uint decalLane0;
    fastPath = IsFastPath(decalStart, decalLane0);

    if (fastPath)
    {
        decalStart = decalLane0;
    }
#endif
