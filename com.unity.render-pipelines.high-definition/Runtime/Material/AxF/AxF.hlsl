...
uint GetEnvSamplingNumOfSamples()
{
    ...
    {
        uint quality = abs(GetEnvSamplingModeQuality());
...
PreLightData    GetPreLightData(float3 viewWS_Clearcoat, PositionInputs posInput, inout BSDFData bsdfData)
{
    ...
    // todo_dir: original code drop, nonsense:
    ...
    // todo_dir todo_modes todo_pseudorefract: cant use undercoat like that
...
{
    //todo_envsampling: cleanup, dedupe with carpaint and add keyword for clearcoat to prune runtime cost.
                              ...
                              int _GPUImageBasedLightingType)
{
    ...
    if (sampleCount > 0)
    {
        ...
        if (AxFEnvSamplingLobeAlloc != AXF_ENV_SAMPLING_LOBE_ALLOC_ALL)
        {
            ...
            //tocheck_envsampling: provide uniforms for the single lobe sampler if we go that way
