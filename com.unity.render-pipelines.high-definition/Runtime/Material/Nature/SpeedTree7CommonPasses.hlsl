...
#ifdef EFFECT_BUMP
    output.tangentToWorld = BuildTangentToWorld(input.interpolators2, input.interpolators1.xyz);
#else
    output.tangentToWorld = BuildTangentToWorld(input.interpolators2, input.interpolators1.xyz);
#endif
    output.tangentToWorld[0] = GetOddNegativeScale() * cross(output.tangentToWorld[2], output.tangentToWorld[1]);
#endif
