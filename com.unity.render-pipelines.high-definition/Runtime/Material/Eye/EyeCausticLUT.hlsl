...
float ComputeCausticFromLUT(float2 irisPlanePosition, float irisHeight, float3 lightPosOS)
{
    ...
    //these could be exposed for tuning, for now hardcoded
    float causticIntensityMultiplier = 2.f;
    float causticBlend = 1.0f;
    ...
    float illuminance = SAMPLE_TEXTURE3D_LOD(_PreIntegratedEyeCaustic, _CausticLUT_trilinear_clamp_sampler, float3(uv.x, uv.y,1.f - w), 0).x;
    ...
    illuminance *= bc.x * bc.y * smoothstep(-0.2f, 0.0f, uv.x);

    illuminance *= causticIntensityMultiplier * blendToBlack;
    caustic *= lerp(1.f, illuminance, causticBlend);
