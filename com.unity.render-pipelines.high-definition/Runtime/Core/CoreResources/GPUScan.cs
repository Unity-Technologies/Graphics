...
namespace UnityEngine.Rendering.HighDefinition
{
    ...
    class GPUScan
    {
        ...
        /// <returns>A RTHandle which contains the results of the performed scan. The lifetime of the RTHandle is managed by the user (cf. RTHandlesDeleter.ScheduleRelease(...), myRTHandle.Release()).</returns>
        static public RTHandle ComputeOperation(RTHandle input, CommandBuffer cmd, Operation operation, Direction direction, GraphicsFormat outFormat = GraphicsFormat.None)
        {
            ...
            RTHandlesDeleter.ScheduleRelease(temp0);
            RTHandlesDeleter.ScheduleRelease(temp1);
            ...
            else if (operation == Operation.Total || operation == Operation.MinMax)
            {
                ...
                if (direction == Direction.Horizontal)
                {
                    ...
                    RTHandlesDeleter.ScheduleRelease(cdf);
                    if (cmd != null)
                    {
