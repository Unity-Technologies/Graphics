...
{
    return Rescale01(x, xmin, xmax)*(newMax - newMin) + newMin;
}
...
float4 fragArray(v2f i) : SV_Target
{
    ...
    float3 output;
    if (_buildPDF == 1)
        output = (float3)SampleToPDFMeasure(UNITY_SAMPLE_TEXCUBEARRAY_LOD(_srcCubeTextureArray, float4(dir, _cubeArrayIndex), (float)_cubeMipLvl).rgb).xxx;
    else
        output = (float3)UNITY_SAMPLE_TEXCUBEARRAY_LOD(_srcCubeTextureArray, float4(dir, _cubeArrayIndex), (float)_cubeMipLvl).rgb;
