...
float2 DirectionToSphericalTexCoordinate(float3 dir_in) // use this for the lookup
{
    ...
    float recipPi = 1.0f / UNITY_PI;
    return float2( 1.0f - 0.5f * recipPi * atan2(dir.x, -dir.z), asin(dir.y) * recipPi + 0.5f );
}

float3 SphericalTexCoordinateToDirection(float2 sphTexCoord)
{
    float theta = (1.0f - sphTexCoord.x) * (2.0f * UNITY_PI);
    float phi = (sphTexCoord.y - 0.5f) * UNITY_PI;
...
{
    return dot(value.rgb, float3(1.0f / 3.0f, 1.0f / 3.0f, 1.0f / 3.0f));
}
...
float4 frag(v2f i) : SV_Target
{
    ...
    float angle = i.texcoord.y * UNITY_PI;
...
float4 fragArray(v2f i) : SV_Target
{
    ...
    float angle = (1.0f - i.texcoord.y) * UNITY_PI;
