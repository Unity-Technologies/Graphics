...
| **Property**                  | **Description**                                              |
| ----------------------------- | ------------------------------------------------------------ |
| **Use Opacity as Density Blend** | Specifies whether HDRP should render the Terrain Layer using the value stored in the alpha channel of the Terrain Layer's **Diffuse Map** Texture instead of the usual splatmap weight or height value from the **Mask Map**. The alpha channel value is used as a threshold value for the layer blending. If the splat weight value combined with the alpha channel value is greater than one, the Terrain Layer will be visible as part of the blended result. After this calculation, the modified weights are then normalized in the shader. |
