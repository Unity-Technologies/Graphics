...
Unity creates a Project and automatically installs the HDRP package, and all of its dependencies.
...
To find the right balance between render quality and run-time performance, adjust the [Frame Settings](Frame-Settings.html) for your [Cameras](HDRP-Camera.html). The Frame Settings available to you depend on which settings you enable in the HDRP Asset. You can change the Frame Settings at run time.
...
HDRP enables you to create Materials like glass, that refract light based on definable options. The options for a Material depend on which Shader the Material uses. HDRP shares many Material properties across Shaders. For more information, see [HDRP Material features](HDRP-Features.html#Material).
...
HDRP adds settings to the [Light Explorer](https://docs.unity3d.com/Manual/LightingExplorer.html) (menu: **Window > General > Light Explorer**) so that you can adjust the HDRP features and lighting units. ![](Images/GettingStarted4.png)
