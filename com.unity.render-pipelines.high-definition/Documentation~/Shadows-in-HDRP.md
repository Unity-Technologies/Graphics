...
Note that HDRP can perform dynamic rescale of the shadow maps in a way that the smaller the area of the screen that the light covers the more the resolution is scaled down. This can be useful to maximize the usage of the space in the shadow atlases, but also to reduce the impact on performance.  This option can be activated in the shadow section of the your Unity Project’s [HDRP Asset](HDRP-Asset.md).
