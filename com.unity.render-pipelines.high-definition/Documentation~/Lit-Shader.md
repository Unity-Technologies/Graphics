...
| **Force Forward Emissive** | Enable the checkbox to render Emissive contibution of this Material in a separate forward pass in the case the Lit Shader Mode is setup to Both or Deferred. This can be used to fix artefacts when using [Screen Space Global Illumination](Override-Screen-Space-GI.md) - With or without Raytracing enabled - and Emissive Material with Lit Shader Mode setup as Both or Deferred. Previously it Emissive contributoin was drop, now it is keep. Same usage for the new Adaptive probe volumes.
