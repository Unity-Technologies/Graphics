...
| **Runtime Debug Display**               | Enable the checkbox to make HDRP being able to use debug mode from RenderPipeline windows at runtime. Disable this checkbox to reduce build time and Shader memory. This disables the following debug modes: All material properties debug modes except GBuffer debug. The various properties overrides options. All the lighting debug modes.  |
| **Runtime AOV API**                     | Enable the checkbox to make HDRP being able to use AOV API (Rendering of Material properties an lighting mode) at runtime. Disable this checkbox to reduce build time and Shader memory. This disables the following AOV modes: All material properties and Lighting modes. |
