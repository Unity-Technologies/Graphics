...
The Influence Volume defines the area around the Probe in which reflective Materials use the results that the Probe captures to influence the reflective behavior of their surface.
...
| **Per Axis Control**      | Enable the checkbox to control the **Blend Distance** and **Blend Normal Distance** per axis. Only available with a **Box Shape**. |
...
| ![](Images/ReflectionProbeGizmo6.png) | **Chrome Gizmo**.                     | Displays a chrome sphere to preview the probe's texture in the scene. |
