...
| **Roughness Bias Factor** | Controls the relative roughness offset. A low value means material roughness stays the same, a high value means smoother reflections.<br/>This property only appears if you set **Algorithm** to **PBR Accumulation**. |
| **Speed Rejection Factor**  | Controls the likelihood history will be rejected based on the previous frame motion vectors (both the surface and the hit object).<br/>This property only appears if you set **Algorithm** to **PBR Accumulation**. |
