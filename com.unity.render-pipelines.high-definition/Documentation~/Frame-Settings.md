...
Note that these settings will have an effect only if asynchronous compute is supported by the target platform. Also please note that currently it is also not enabled for D3D12. 
