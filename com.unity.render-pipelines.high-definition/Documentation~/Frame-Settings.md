...
|**Asymmetric Projection**            |Enable the checkbox to allow HDRP to account for asymmetric projection when evaluating the view direction based on pixel coordinates.|
|**Screen Coordinates Override**      |Enable the checkbox to allow HDRP to use Screen Coordinates Override for post processing and custom passes. This allows post effects to be compatible with Cluster Display for example.|
