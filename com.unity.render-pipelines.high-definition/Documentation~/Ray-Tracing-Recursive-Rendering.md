...
| **Ray Miss**         | Defines if HDRP should use the reflection probes, the sky, both or nothing as a fall-back for recursive rendering when a ray doesn't find an intersection. |
| **Last Bounce**      | Defines if HDRP should use the reflection probes, the sky, both or nothing as a fall-back for recursive rendering when lighting the last bounce. |
