...
| **IES Profile**                   | An IES File that describe the light profile. If a cookie and an IES profile are setup, a linear average of them is used. When we have at the same time a cookie and an IES Profile during a light map baking, only the cookie will be used. |
