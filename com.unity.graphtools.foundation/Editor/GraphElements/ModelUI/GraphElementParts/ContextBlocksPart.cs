...
namespace UnityEditor.GraphToolsFoundation.Overdrive
{
    ...
    public class ContextBlocksPart : BaseModelUIPart
    {
        ...
        protected override void BuildPartUI(VisualElement container)
        {
            ...
            m_AddBlock.RegisterCallback<MouseDownEvent>(e => ((ContextNode)m_OwnerElement).DisplaySmartSearch(e.mousePosition));
        ...
        protected override void UpdatePartFromModel()
        {
            ...
            List<ModelUI> blocks = Root.Children().OfType<ModelUI>().ToList();
            ...
            var exisitingModels = new HashSet<IGraphElementModel>(blocks.Select(u => u.Model));
            ...
            foreach (var blockModel in orderBlockModels)
            {
                ...
                ModelUI newBlockNode = GraphElementFactory.CreateUI<ModelUI>(m_OwnerElement.View, blockModel.Value);

                newBlockNode.AddToView(m_OwnerElement.View);
            ...
            blocks = Root.Children().OfType<ModelUI>().ToList();
            ModelUI firstBlockNode = blocks.LastOrDefault();
            IBlockNodeModel firstModel = blockModels[0];
            if (firstBlockNode == null || firstBlockNode.Model != firstModel)
            {
                firstBlockNode = blocks.Last(t => t.Model == firstModel);
                firstBlockNode.PlaceBehind(m_Etch);
                blocks.Remove(firstBlockNode);
                blocks.Insert(blocks.Count - 1, firstBlockNode);
            }

            ModelUI prevBlockNode = firstBlockNode;
            for (int i = 1; i < blockModels.Count; ++i)
            {
                ModelUI currentBlockNode = blocks.First(t => t.Model == blockModels[i]);
            ...
            foreach (ModelUI block in blocks)
