...
{
    /// <summary>
    /// The ModelUI class for block nodes.
    /// </summary>
    public class BlockNode : CollapsibleInOutNode
    {
        ...
        /// </summary>
        public IBlockNodeModel BlockNodeModel => base.Model as IBlockNodeModel;
        ...
        public override bool PasteIn(PasteOperation operation, string operationName, Vector2 delta, CopyPasteData copyPasteData)
        {
            ...
            if (operation == PasteOperation.Duplicate)
            {
                ...
                var selection = this.GraphView.GetSelection();
                ...
                foreach (var contextData in groupedBlocks)
                {
                    ...
                    {
                        m_Context = contextData.Key,
                        m_Blocks = contextData.ToList(),
                        m_Index = contextData.Max(t => t.GetIndex()) + 1
                    };
                }
            ...
            {
                // If we paste, we paste everything bellow the first selected block
                GraphView.Dispatch(new InsertBlocksInContextCommand(BlockNodeModel.ContextNodeModel, copyPasteData.nodes.OfType<IBlockNodeModel>().Where(t => t.ContextNodeModel == BlockNodeModel.ContextNodeModel).Max(t => t.GetIndex()), copyPasteData.nodes.OfType<IBlockNodeModel>(), true, operationName));
            }
