...
namespace UnityEditor.GraphToolsFoundation.Overdrive
{
    ...
    public class CreateVariableNodesCommand : UndoableCommand
    {
        ...
        public static void DefaultCommandHandler(UndoStateComponent undoState, GraphViewStateComponent graphViewState, CreateVariableNodesCommand command)
        {
            var graphModel = graphViewState.GraphModel;
            var stencil = graphModel.Stencil as Stencil;

            var variablesToAdd = command.VariablesToCreate
                .Where(v => stencil == null || stencil.CanCreateVariableInGraph(v.Item1, graphModel))
                .ToList();
            ...
            if (variablesToAdd.Count > 0)
            {
                ...
                using (var graphUpdater = graphViewState.UpdateScope)
                {
                    ...
                    foreach (var(variableDeclarationModel, guid, position) in variablesToAdd)
                    {

                        var newVariable = graphModel.CreateVariableNode(variableDeclarationModel, position, guid: guid);
    ...
    public class ConvertConstantNodesAndVariableNodesCommand : UndoableCommand
    {
        ...
        public static void DefaultCommandHandler(UndoStateComponent undoState, GraphViewStateComponent graphViewState, SelectionStateComponent selectionState, ConvertConstantNodesAndVariableNodesCommand command)
        {
            ...
            using (var selectionUpdater = selectionState.UpdateScope)
            {
                ...
                foreach (var constantModel in command.ConstantNodeModels ?? Enumerable.Empty<IConstantNodeModel>())
                {
                    var declarationModel = graphModel.CreateGraphVariableDeclaration(
                        ...
                        true, null, -1, constantModel.Value.CloneConstant());
    ...
    public class ItemizeNodeCommand : ModelCommand<ISingleOutputPortNodeModel>
    {
        ...
        /// <param name="graphViewState">The graph view state component.</param>
        /// <param name="command">The command.</param>
        public static void DefaultCommandHandler(UndoStateComponent undoState, GraphViewStateComponent graphViewState, ItemizeNodeCommand command)
        {
    ...
    public class LockConstantNodeCommand : ModelCommand<IConstantNodeModel, bool>
    {
        ...
        /// <param name="graphViewState">The graph view state component.</param>
        /// <param name="command">The command.</param>
        public static void DefaultCommandHandler(UndoStateComponent undoState, GraphViewStateComponent graphViewState, LockConstantNodeCommand command)
        {
    ...
    public class ChangeVariableDeclarationCommand : ModelCommand<IVariableNodeModel>
    {
        ...
        /// <param name="graphViewState">The graph view state component.</param>
        /// <param name="command">The command.</param>
        public static void DefaultCommandHandler(UndoStateComponent undoState, GraphViewStateComponent graphViewState, ChangeVariableDeclarationCommand command)
        {
            if (!command.Models.Any())
                return;
            ...
            using (var graphUpdater = graphViewState.UpdateScope)
            {
                foreach (var model in command.Models)
                {
                    ...
                    var references = graphViewState.GraphModel.FindReferencesInGraph<IVariableNodeModel>(command.Variable);
