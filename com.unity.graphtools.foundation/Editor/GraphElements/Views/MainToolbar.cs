...
namespace UnityEditor.GraphToolsFoundation.Overdrive
{
    ...
    public class MainToolbar : Toolbar
    {
        ...
        public MainToolbar(BaseGraphTool graphTool, GraphView graphView) : base(graphTool, graphView)
        {
            ...
            m_EnableTracingButton.RegisterValueChangedCallback(e => GraphTool?.Dispatch(new ActivateTracingCommand(e.newValue)));
        ...
        protected virtual void OnBreadcrumbClick(OpenedGraph graphToLoad, int breadcrumbIndex)
        {
            if (graphToLoad.GetGraphAssetModel()?.FriendlyScriptName != null)
                GraphTool?.Dispatch(new LoadGraphAssetCommand(graphToLoad.GetGraphAssetModelPath(), graphToLoad.AssetLocalId, GraphView.Window.PluginRepository,
        ...
        protected virtual void UpdateCommonMenu()
        {
            ...
            var stencil = (Stencil)GraphTool?.ToolState.GraphModel?.Stencil;
        ...
        protected virtual void BuildOptionMenu(GenericMenu menu)
        {
            ...
            if (prefs != null)
            {
                if (Unsupported.IsDeveloperMode())
                {
                        ...
                        if (GraphTool?.ToolState.GraphModel != null)
                        {
                            ...
                            GraphTool?.Dispatch(
                                new LoadGraphAssetCommand(openedGraph.GetGraphAssetModelPath(),
                                    openedGraph.AssetLocalId, GraphView.Window.PluginRepository));
                        }
                    });
                    ...
                    {
                        GraphView.GetBlackboard()?.BuildUI();
                        GraphView.GetBlackboard()?.UpdateFromModel();
                    });
                }
            }
        }
