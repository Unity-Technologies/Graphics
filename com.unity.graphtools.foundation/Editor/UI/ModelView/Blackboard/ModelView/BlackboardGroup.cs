...
{
    /// <summary>
    /// A BlackboardElement to display a <see cref="IGroupModel"/>.
    /// </summary>
    public class BlackboardGroup : BlackboardElement, IModelViewContainer
    {
        /// <summary>
        ...
        public IEnumerable<IModelView> ModelViews
        {
            get
            {
                foreach (var element in m_ItemsContainer.Children().OfType<IModelView>())
                {
        ...
        {
            RootView.Dispatch(new ExpandVariableGroupCommand(GroupModel, e.newValue));
        }
        ...
        static void RecurseGetVariables(IEnumerable<IGroupItemModel> items, List<IVariableDeclarationModel> variables)
        {
        ...
        public virtual bool CanAcceptDrop(IEnumerable<IGraphElementModel> draggedObjects)
        {
            ...
            var variables = new List<IVariableDeclarationModel>();
        ...
        /// <param name="elements">The dropped <see cref="IGraphElementModel"/>s.</param>
        protected void OnItemDropped(int index, IEnumerable<IGraphElementModel> elements)
        {
            ...
            droppedModels.Sort(GroupItemOrderComparer.Default);
            ...
            RootView.Dispatch(new ReorderGroupItemsCommand((IGroupModel)Model, insertAfterModel, droppedModels));
        }
        ...
        public override bool HandlePasteOperation(PasteOperation operation, string operationName, Vector2 delta, CopyPasteData copyPasteData)
        {
            ...
            {
                BlackboardView.Dispatcher.Dispatch(new PasteSerializedDataCommand(operation, operationName, delta, copyPasteData, GroupModel));
                return true;
            }
