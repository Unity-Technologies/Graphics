...
namespace UnityEditor.GraphToolsFoundation.Overdrive
{
    ...
    public abstract class GraphViewToolWindow : GraphViewToolWindowBridge
    {
        ...
        protected UIElements.Toolbar m_Toolbar;
        ...
        protected GraphView SelectedGraphView { get; private set; }
        ...
        protected virtual void Update()
        {
            ...
            if (m_FirstUpdate)
            {
                ...
                if (m_SelectedWindow != null)
                {
                    ...
                    if (graphViewEditor != null && m_SelectedGraphViewIdx >= 0 && SelectedGraphView == null)
                    {
                        SelectedGraphView = graphViewEditor.GraphViews.ElementAt(m_SelectedGraphViewIdx);
                        OnGraphViewChanged();
                    }
                }
            }
            ...
            {
                if (!m_SelectedWindow && SelectedGraphView != null)
        ...
        void RefreshPanelChoices()
        {
            ...
            {
                if (toolWindow.SelectedGraphView != null)
                {
                    usedGraphViews.Add(toolWindow.SelectedGraphView);
                }
            }
            ...
            {
                int idx = 0;
                foreach (var graphView in window.GraphViews)
                {
            ...
            foreach (var graphView in m_GraphViewChoices)
            {
                menu.AppendAction(GetDisplayName(graphView.graphView), OnSelectGraphView,
                    a =>
                    {
                        ...
                        return (gvc.graphView == SelectedGraphView
        ...
        void OnSelectGraphView(DropdownMenuAction action)
        {
            ...
            if (newlySelectedGraphView == SelectedGraphView)
                return;
        ...
        {
            OnGraphViewChanging();
            SelectedGraphView = choice?.graphView;
        ...
        {
            string updatedName = GetDisplayName(SelectedGraphView);

            if (m_SelectorMenu.text != updatedName)
                m_SelectorMenu.text = updatedName;
        }

        static string GetDisplayName(GraphView gv)
        {
