...
namespace UnityEditor.GraphToolsFoundation.Overdrive
{
    class AutoSpacingHelper : AutoPlacementHelper
    {
        ...
        public void SendSpacingCommand(PortOrientation orientation)
        {
            ...
            Dictionary<IModel, Vector2> results = GetElementDeltaResults();

            // Dispatch command
            SendPlacementCommand(results.Keys.ToList(), results.Values.ToList());
        }

        protected override float GetStartingPosition(List<Tuple<Rect, List<IModel>>> boundingRects)
        {
        ...
        protected override List<Tuple<Rect, List<IModel>>> GetBoundingRectsList(List<Tuple<Rect, List<IModel>>> boundingRects)
        {
