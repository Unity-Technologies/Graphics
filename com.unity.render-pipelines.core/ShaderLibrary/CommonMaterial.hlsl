...
// This macro evaluates the local frame based on its world space normal that should be used by a given point. 
// To avoid breaking the API by adding new mandatory functions in the requirements, this macro doesn't need to be
// overriden by the various materials, but can be in case the local frame is different from the default one generated by the normal alone.
#define GET_LOCAL_FRAME_MATERIAL(bsdfData, normalWS, localFrame) {localFrame = GetLocalFrame(normalWS);}
