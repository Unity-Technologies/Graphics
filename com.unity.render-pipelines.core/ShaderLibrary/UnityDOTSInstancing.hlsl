...
float4 LoadDOTSInstancedData_SelectionValue(uint metadata, uint submeshIndex, float4 globalSelectionID)
{
    ...
    if (IsDOTSInstancedProperty(metadata))
    {
        ...
        static const uint EntityIDBitCount = 26;

        static const uint SubmeshIndexBitsAndMask = (1 << SubmeshIndexBitCount) - 1;
        static const uint EntityIDBitsAndMask = (1 << EntityIDBitCount) - 1;
        ...
        uint submeshBits = (submeshIndex & SubmeshIndexBitsAndMask) << EntityIDBitCount;
        uint idValueBits = idValue & EntityIDBitsAndMask;
