    ...
    public partial class UIDefinition
    {
        public struct CategoryId : IEquatable<CategoryId>
        {
            ...
            )
            {
                ...
                {
                    error = new ArgumentException($"{nameof(CategoryId)} must not be empty").WithStackTrace();
                    CategoryId = Empty;
                    return false;
                }
        ...
        public struct PropertyPath : IEquatable<PropertyPath>
        {
            ...
            )
            {
                if (string.IsNullOrEmpty(name))
                {
                    error = new ArgumentException($"{nameof(PropertyPath)} must not be empty").WithStackTrace();
                    PropertyPath = Empty;
                    return false;
                }
        ...
        public struct PropertyName : IEquatable<PropertyName>
        {
            ...
            )
            {
                if (string.IsNullOrEmpty(name))
                {
                    error = new ArgumentException($"{nameof(PropertyName)} must not be empty").WithStackTrace();
                    PropertyName = Empty;
                    return false;
                }
        ...
        public struct PropertyTooltip : IEquatable<PropertyTooltip>
        {
            ...
            )
            {
                if (string.IsNullOrEmpty(name))
                {
                    error = new ArgumentException($"{nameof(PropertyTooltip)} must not be empty").WithStackTrace();
