...
namespace UnityEngine.Rendering.MeshDecal
{
    public partial class MeshDecalProjector : MonoBehaviour
    {
        ...
        var meshDataArray = Mesh.AcquireReadOnlyMeshData(sourceMeshBuffer);
            ...
            // Generate mesh
            var result = GenerateMeshData(meshDataArray, matrixList, size, offset, new float4(m_uvRect.x, m_uvRect.y, m_uvRect.width, m_uvRect.height), normalBlend);

            meshDataArray.Dispose();
            matrixList.Dispose();

            // Apply to mesh
            Mesh.ApplyAndDisposeWritableMeshData(result, mesh);
        ...
        /// <summary>
        /// Combines meshes transformed by specified matrices, cuts triangles off with specified size and creates a MeshDataArray.
        /// </summary>
        static Mesh.MeshDataArray GenerateMeshData(Mesh.MeshDataArray sourceMeshes, NativeList<float4x4> matrices,
            float3 boxSize, float backOffset, float4 uvRect, float normalBlend)
        {
            // Allocate temp buffers
            var meshBuffers = new MeshBuffers(3, Allocator.TempJob);

            // Fill temp buffers
            new FillMeshBuffersJob
            {
            ...
            // Allocate output mesh data
            // Uses managed array for VertexAttributeDescriptors so can't compile it with Burst
            var outputArray = Mesh.AllocateWritableMeshData(1);
            var outputMesh = outputArray[0];
            outputMesh.SetVertexBufferParams(meshBuffers.vertices.Length,
                new VertexAttributeDescriptor(VertexAttribute.Position),
                new VertexAttributeDescriptor(VertexAttribute.Normal, stream: 1),
                new VertexAttributeDescriptor(VertexAttribute.TexCoord0, stream: 2, dimension: 2),
                new VertexAttributeDescriptor(VertexAttribute.Tangent, stream: 3, dimension: 4));

            // Fill final mesh data from buffers
            new FillMeshDataJob
            {
                buffers = meshBuffers,
                boxSize = boxSize,
        ...
        [BurstCompile]
        struct FillMeshBuffersJob : IJob
        {
            ...
            public MeshBuffers output;

            public void Execute()
            {
                FillMeshBuffers(sourceMeshes, matrices, boxSize, backOffset, normalBlend, output);
            }
        }
        ...
        /// </summary>
        static void FillMeshBuffers(Mesh.MeshDataArray sourceMeshes, NativeArray<float4x4> matrices,
        ...
        {
            [ReadOnly] public MeshBuffers buffers;
            public float3 boxSize;
            ...
            public void Execute()
            {
                FillUVsAndCopyIndices(buffers, boxSize, uvRect, output);
            }
        }

        /// <summary>
        /// Generates UVs and writes vertex data into mesh data. Burst-friendly.
        /// </summary>
        static void FillUVsAndCopyIndices(MeshBuffers buffers, float3 boxSize, float4 uvRect, Mesh.MeshData outputMesh)
        {
            ...
            // Generate UVs
            var uvs = outputMesh.GetVertexData<float2>(2);
            ...
            // Generate tangents
            var tangents = outputMesh.GetVertexData<float4>(3);
            for (var i = 0; i < vertexCount; i++)
