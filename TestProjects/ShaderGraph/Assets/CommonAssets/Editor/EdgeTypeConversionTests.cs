    ...
    internal class EdgeTypeConversionTests
    {
        ...
        public void TestAllCombos()
        {
            foreach (var slot in m_CFNode.GetInputSlots<MaterialSlot>())
            {
                ...
                var edge = m_Graph.GetEdges(slot).FirstOrDefault();
                if (edge == null) continue;

                var outputNode = edge.outputSlot.owner;
                var outputSlot = outputNode.GetOutputSlots<MaterialSlot>().First(s => s == edge.outputSlot);
                ...
                {
                    var newSlot = MaterialSlot.CreateMaterialSlot(slotValType, slot.slotId, slot.displayName, slot.shaderOutputName,
                    ...
                    {
                        if (message.Key.Equals(m_CFNode) && message.Value.Exists(msg =>
