...
public class HDRPShowLightCluster : MonoBehaviour
{
        ...
        for (int x = 0; x < rows; x++)
        {
            for (int y = 0; y < cols; y++)
            {
                ...
                // for ( int i = 0; i < 4; i++ )
                {
                    ...
                    Debug.DrawLine( n, f, heightColor, Time.deltaTime, true ); //sides of the created projection matrix
                }
            }
        }
    }
    ...
    static void DrawFrustum2(Camera cam)
    {
        ...
        for ( int i = 0; i < 4; i++ ) {
            Debug.DrawLine( nearCorners[i], nearCorners[( i + 1 ) % 4], widthColor, Time.deltaTime, true ); //near corners on the created projection matrix
            Debug.DrawLine( farCorners[i], farCorners[( i + 1 ) % 4], depthColor, Time.deltaTime, true ); //far corners on the created projection matrix
            Debug.DrawLine( nearCorners[i], farCorners[i], heightColor, Time.deltaTime, true ); //sides of the created projection matrix
        }
    }
