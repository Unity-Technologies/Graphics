...
namespace UnityEditor.ShaderFoundry.UnitTests
{
    class UInt4PropertyTests : BlockTestRenderer
    {
        ...
        [UnityTest]
        public IEnumerator UInt4Property_DefaultPropertyValueUsed_IsExpectedColor()
        {
            var expectedColor = new Color(1, 1, 1, 1);

            var container = CreateContainer();
            var propBuilder = BuildWithoutNameOverrides("(1, 1, 1, 1)");
            var block = propBuilder.Build(container, container._uint4);
        ...
        [UnityTest]
        public IEnumerator UInt4Property_MaterialColorSet_IsExpectedColor()
        {
